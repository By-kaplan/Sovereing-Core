...
<head>
    ...
    <title>NEFER-OS | 1.0.0-OMEGA | 89. HÜCRE (RESOURCE REDISTRIBUTION)</title>
    <style>
        :root {
            --bg-deep: #020617;
            --panel-bg: #0f172a;
            ...
            --shadow-net: #22d3ee;
            --resource-redist: #34d399; /* 89. Hücre Rengi: Zümrüt Yeşili / Yaşam Destek */
            --border: #1e293b;
            --cell-gap: 4px;
        }
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; cursor: crosshair; }
        ...
        .nefer-header {
            ...
            align-items: center;
            border-left: 4px solid var(--resource-redist);
        }

        .nefer-header h1 { font-size: 1.2rem; color: var(--resource-redist); letter-spacing: 3px; font-weight: 800; }
        ...
        .cell.resource-active { background: var(--resource-redist); box-shadow: 0 0 15px var(--resource-redist); animation: flow-pulse 1.5s infinite ease-in-out; }
        .cell.loading { background: var(--warning); animation: pulse 1s infinite; }

        @keyframes flow-pulse { 0%, 100% { opacity: 1; transform: scale(1); } 50% { opacity: 0.6; transform: scale(1.1); } }
        ...
        .viz-view {
            ...
            grid-template-rows: repeat(176, 35px) 1fr;
            gap: 2px;
            overflow-y: auto;
        }
        ...
        .log-entry.shadow-net { border-left-color: var(--shadow-net); color: var(--shadow-net); }
        .log-entry.resource { border-left-color: var(--resource-redist); color: var(--resource-redist); font-weight: bold; background: rgba(52, 211, 153, 0.1); }
        ...
        .btn.active-op-resource { border-color: var(--resource-redist); background: #064e3b; color: white; border-width: 2px; }
...
<body>
    ...
    <header class="nefer-header">
        <div>
            <h1>NEFER-OS // ALGORITHMIC_RESOURCE_REDISTRIBUTION</h1>
            <div style="font-size: 0.6rem; color: var(--text-dim);">89. HÜCRE: ALGORİTMİK KAYNAK YENİDEN DAĞITIMI VE YAŞAM DESTEĞİ</div>
        </div>
        <div class="meta-info">
    ...
    <div class="console-container">
        <div class="main-display">
            <div class="viz-view" id="viz-container">
                ...
                <!-- 89. Hücre: Resource Redistribution Vizüalizasyonu -->
                <div class="viz-box" style="grid-column: span 6; grid-row: span 4;">
                    <div style="position:absolute; top:5px; left:10px; font-size:0.4rem; color:var(--resource-redist); z-index:5;">COLLECTIVE_LIFE_SUPPORT_ACCOUNTING_ENGINE</div>
                    <canvas id="resource-redist-canvas"></canvas>
                    <div id="resource-hud" class="hud-text" style="color: var(--resource-redist);">CREDITS: 14.2M | TRANSFERS: ACTIVE | AVG_LOAD: 32% | STATE: BALANCING</div>
                </div>
            </div>
        ...
        <aside class="tools-panel">
            ...
            <button class="btn" id="resource-toggle" onclick="ResourceRedistributionEngine.toggle()">
                ...
                <span id="resource-status">KAPALI</span>
            </button>
            ...
            <div style="margin-top: auto; font-size: 0.55rem; color: var(--text-dim); border-top: 1px solid var(--border); padding-top: 10px;">
                HÜCRE 89: Sığınaklar arası enerji ve işlem gücü transferini otonom yönetir.
            </div>
        </aside>
    </div>
    ...
    <footer class="protocol-footer">
        <div class="attribution">
            <p><b>89. HÜCRE: RESOURCE REDISTRIBUTION</b> - Kolektif Ekonomi Protokolü</p>
            <p>Architect: Ömer Kaplan | Organization: Kaplan Precision Dept.</p>
        </div>
        <div class="flag-box">
            <svg width="100" height="60" viewBox="0 0 150 100">
                <defs><mask id="m"><mask id="m"><rect width="150" height="100" fill="white"/><circle cx="61.25" cy="50" r="20" fill="black"/></mask></defs>
    ...
    <script type="module">
        const Kernel = {
            init() {
                ...
                this.log("Kernel aktif. Kaynak Yeniden Dağıtımı (Resource Redist) modülü mühürlendi.", "boot");
            },
            setupGrid() {
            ...
            async sequenceBoot() {
                const colors = ['active', 'boot-active', 'resource-active'];
                for(let i=1; i<=89; i++) {
                ...
                this.log("89 Hücre kolektif yaşam desteği için senkronize edildi.", "sys");
            }
        };
        ...
        const ResourceRedistributionEngine = {
            ...
            toggle() {
                ...
                const statusText = document.getElementById('resource-status');
                const hud = document.getElementById('resource-hud');

                if(this.active) {
                    if(btn) btn.classList.add('active-op-resource');
                    if(statusText) statusText.textContent = "DENGELİ";
                    Kernel.updateCell(89, "resource-active");
                    Kernel.log("KAYNAK DAĞITIMI AKTİF: SIĞINAKLAR ARASI ENERJİ HAVUZU OLUŞTURULDU.", "resource");
                    this.start();
                } else {
                    if(btn) btn.classList.remove('active-op-resource');
                    if(statusText) statusText.textContent = "KAPALI";
                    Kernel.updateCell(89, "active");
                    cancelAnimationFrame(this.loop);
                    if(hud) hud.textContent = "CREDITS: -- | TRANSFERS: OFF | STATE: IDLE";
                    Kernel.log("Kaynak muhasebe motoru durduruldu.", "sys");
                }
            },

            start() {
                const c = document.getElementById('resource-redist-canvas');
                if(!c) return;
                const ctx = c.getContext('2d');
                c.width = c.offsetWidth; c.height = c.offsetHeight;

                this.shelters = Array.from({length: 10}, () => ({
                ...
                const draw = () => {
                    ...
                    const hud = document.getElementById('resource-hud');
                    if(hud) hud.textContent = `CREDITS: ${Math.floor(Math.random()*100)}M | TRANSFERS: ${this.transfers.length} | STATE: COLLECTIVE_STABILITY`;

                    // Sığınakların Durumu (Enerji ve Kredi)
                    this.shelters.forEach((s, idx) => {
                        s.energy += (Math.random() - 0.5) * 2;
                        if(s.energy < 0) s.energy = 0; if(s.energy > 100) s.energy = 100;

                        ctx.fillStyle = s.energy < 20 ? "var(--hazard)" : "var(--resource-redist)";
                        ctx.beginPath();
                        ...
                        ctx.fillRect(s.x - 10, s.y + 8, 20, 2);
                        ctx.fillStyle = s.energy < 20 ? "var(--hazard)" : "var(--resource-redist)";
                        ctx.fillRect(s.x - 10, s.y + 8, (s.energy / 100) * 20, 2);

                        // Düşük Enerjili Sığınaklara Transfer Tetikleme
                        if(s.energy < 20 && Math.random() > 0.98) {
                            const donor = this.shelters.find(d => d.energy > 60);
                            if(donor) {
                                this.transfers.push({ 
                                    from: donor, to: s, progress: 0, 
                                    id: `TX_${Math.floor(Math.random()*0xFFFF).toString(16)}` 
                                });
                                Kernel.log(`Redistribution: [${donor.credits}C] -> Node_${idx} (Enerji Takviyesi)`, "resource");
                            }
                        }
                    });
                    ...
                    this.transfers.forEach((t, i) => {
                        ...
                        // Hareket eden kredi parçacığı
                        const px = t.from.x + (t.to.x - t.from.x) * t.progress;
                        const py = t.from.y + (t.to.y - t.from.y) * t.progress;
                        ctx.fillStyle = "white";
        ...
        const ShadowNetEngine = {
            ...
            toggle() {
                ...
                Kernel.updateCell(88, this.active ? "shadow-net-active" : "active");
                if(this.active) { if(btn) btn.classList.add('active-op-shadow'); this.start(); } else { if(btn) btn.classList.remove('active-op-shadow'); cancelAnimationFrame(this.loop); }
            },
            start() {
                const c = document.getElementById('shadow-net-canvas'); if(!c) return;
                ...
                const draw = () => {
                    ...
                    ctx.strokeStyle = "var(--shadow-net)"; ctx.strokeRect(c.width/2-20, c.height/2-20, 40, 40);
                    this.loop = requestAnimationFrame(draw);
                }; draw();
            }
        };
