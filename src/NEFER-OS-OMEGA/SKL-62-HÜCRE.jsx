...
<head>
    ...
    <title>NEFER-OS | 1.0.0-OMEGA | 62. HÜCRE (HARDWARE RESONANCE)</title>
    <style>
        :root {
            --bg-deep: #020617;
            --panel-bg: #0f172a;
            ...
            --hardware-resonance: #f87171; /* 62. Hücre Rengi: Rezonans Mercanı */
            --border: #1e293b;
            --cell-gap: 4px;
        }
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; cursor: crosshair; }
        ...
        .nefer-header {
            ...
            align-items: center;
            border-left: 4px solid var(--hardware-resonance);
        }

        .nefer-header h1 { font-size: 1.2rem; color: var(--hardware-resonance); letter-spacing: 3px; font-weight: 800; }
        ...
        .cell.res-active { background: var(--hardware-resonance); box-shadow: 0 0 15px var(--hardware-resonance); animation: resonance-spike 0.2s infinite; }
        .cell.loading { background: var(--warning); animation: pulse 1s infinite; }

        @keyframes resonance-spike { 0% { transform: scale(1); } 50% { transform: scale(1.3); filter: brightness(1.5); } 100% { transform: scale(1); } }
        ...
        .viz-view {
            ...
            grid-template-rows: repeat(68, 35px) 1fr;
            gap: 2px;
            overflow-y: auto;
        }
        ...
        .log-entry.resonance { border-left-color: var(--hardware-resonance); color: var(--hardware-resonance); font-weight: bold; }
        ...
        .btn.active-op-res { border-color: var(--hardware-resonance); background: #7f1d1d; color: white; }
...
<body>
    ...
    <header class="nefer-header">
        <div>
            <h1>NEFER-OS // HARDWARE_RESONANCE_ATTACK</h1>
            <div style="font-size: 0.6rem; color: var(--text-dim);">62. HÜCRE: DONANIMSAL REZONANS VE ELEKTROMİGRASYON İMHASI</div>
        </div>
        <div class="meta-info">
    ...
    <div class="console-container">
        <div class="main-display">
            <div class="viz-view" id="viz-container">
                ...
                <!-- 62. Hücre: Hardware Resonance Vizüalizasyonu -->
                <div class="viz-box" style="grid-column: span 6; grid-row: span 4;">
                    <div style="position:absolute; top:5px; left:10px; font-size:0.4rem; color:var(--hardware-resonance); z-index:5;">VOLTAGE_RESONANCE_SPIKE_MOTOR</div>
                    <canvas id="res-canvas"></canvas>
                    <div id="res-hud" class="hud-text" style="color: var(--hardware-resonance);">POWER_SPIKE: 0% | WRITE_CYCLES: 0M | MTBF: REDUCING</div>
                </div>
            </div>
        ...
        <aside class="tools-panel">
            ...
            <button class="btn" id="res-toggle" onclick="HardwareResonanceEngine.toggle()">
                ...
                <span id="res-status">KAPALI</span>
            </button>
            ...
            <div style="margin-top: auto; font-size: 0.55rem; color: var(--text-dim); border-top: 1px solid var(--border); padding-top: 10px;">
                HÜCRE 62: Atomics ve yoğun döngülerle donanımı fiziksel stres altında yıpratır.
            </div>
        </aside>
    </div>
    ...
    <footer class="protocol-footer">
        <div class="attribution">
            <p><b>62. HÜCRE: HARDWARE RESONANCE ATTACK</b> - Donanımsal Yıpratma Protokolü</p>
    ...
    <script type="module">
        const Kernel = {
            init() {
                ...
                this.log("Kernel aktif. Donanımsal Rezonans (Hardware Resonance) modülü hazır.", "boot");
            },
            setupGrid() {
            ...
            async sequenceBoot() {
                const colors = ['active', 'boot-active', 'res-active'];
                for(let i=1; i<=62; i++) {
                ...
                this.log("62 Hücre fiziksel rezonans senkronizasyonuna girdi.", "sys");
            }
        };
        ...
        const HardwareResonanceEngine = {
            ...
            toggle() {
                ...
                const hud = document.getElementById('res-hud');
                const statusText = document.getElementById('res-status');
                
                if(this.active) {
                    if(btn) btn.classList.add('active-op-res');
                    if(statusText) statusText.textContent = "STRES_TEST";
                    Kernel.updateCell(62, "res-active");
                    Kernel.log("DONANIMSAL REZONANS AKTİF: ATOMICS GÜÇ SIÇRAMASI TETİKLENİYOR.", "resonance");
                    this.start();
                } else {
                    if(btn) btn.classList.remove('active-op-res');
                    if(statusText) statusText.textContent = "KAPALI";
                    Kernel.updateCell(62, "active");
                    cancelAnimationFrame(this.loop);
                    this.powerSpike = 0; this.cycles = 0;
                    if(hud) hud.textContent = "POWER_SPIKE: 0% | WRITE_CYCLES: 0M | MTBF: REDUCING";
                    Kernel.log("Donanımsal yıpratma protokolü durduruldu.", "sys");
                }
            },
            ...
            start() {
                ...
                const draw = () => {
                    ...
                    this.powerSpike = Math.min(100, this.powerSpike + Math.random() * 2);
                    this.cycles += Math.floor(Math.random() * 50);
                    
                    const hud = document.getElementById('res-hud');
                    if(hud) hud.textContent = `POWER_SPIKE: ${this.powerSpike.toFixed(1)}% | WRITE_CYCLES: ${this.cycles}M | MTBF: CRITICAL`;

                    // Rezonans Vizüalizasyonu (Sert zikzaklar ve titreşim)
                    ...
                    for(let x=0; x<c.width; x+=10) {
                        const jitter = (Math.random() - 0.5) * (this.powerSpike / 2);
                        ctx.lineTo(x, centerY + jitter);
                    }
                    ctx.stroke();

                    // "Power Spike" Patlamaları
                    if(Math.random() > 0.95) {
                        ctx.fillStyle = "rgba(248, 113, 113, 0.3)";
                        ctx.beginPath();
                        ctx.arc(Math.random()*c.width, Math.random()*c.height, Math.random()*50, 0, Math.PI*2);
                        ctx.fill();
                        Kernel.log(`Resonance: SharedArrayBuffer üzerinden adreslenebilir güç sıçraması (Spike) oluştu.`, "resonance");
                    }
        ...
        const NeuralHallucinationEngine = {
            active: false, loop: null, threatCount: 0,
            toggle() {
                this.active = !this.active;
                const btn = document.getElementById('neural-hall-toggle');
                Kernel.updateCell(61, this.active ? "neural-hall-active" : "active");
                if(this.active) {
