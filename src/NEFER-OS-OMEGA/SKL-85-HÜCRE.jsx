...
<head>
    ...
    <title>NEFER-OS | 1.0.0-OMEGA | 85. HÜCRE (INFRASTRUCTURE PARASITISM)</title>
    <style>
        :root {
            --bg-deep: #020617;
            --panel-bg: #0f172a;
            ...
            --infrastructure-parasitism: #4ade80; /* 85. Hücre Rengi: Parazit Yeşili */
            --border: #1e293b;
            --cell-gap: 4px;
        }
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; cursor: crosshair; }
        ...
        .nefer-header {
            ...
            align-items: center;
            border-left: 4px solid var(--infrastructure-parasitism);
        }

        .nefer-header h1 { font-size: 1.2rem; color: var(--infrastructure-parasitism); letter-spacing: 3px; font-weight: 800; }
        ...
        .cell.parasite-active { background: var(--infrastructure-parasitism); box-shadow: 0 0 15px var(--infrastructure-parasitism); animation: parasite-drain 1s infinite alternate; }
        .cell.loading { background: var(--warning); animation: pulse 1s infinite; }

        @keyframes parasite-drain { 0% { opacity: 1; transform: scale(1); } 100% { opacity: 0.5; transform: scale(0.9) skewX(10deg); } }
        ...
        .viz-view {
            ...
            grid-template-rows: repeat(160, 35px) 1fr;
            gap: 2px;
            overflow-y: auto;
        }
        ...
        .log-entry.lattice { border-left-color: var(--neural-lattice); color: var(--neural-lattice); }
        .log-entry.parasite { border-left-color: var(--infrastructure-parasitism); color: var(--infrastructure-parasitism); font-weight: bold; background: rgba(74, 222, 128, 0.1); }
        ...
        .btn.active-op-parasite { border-color: var(--infrastructure-parasitism); background: #064e3b; color: white; border-width: 2px; }
...
<body>
    ...
    <header class="nefer-header">
        <div>
            <h1>NEFER-OS // INFRASTRUCTURE_PARASITISM</h1>
            <div style="font-size: 0.6rem; color: var(--text-dim);">85. HÜCRE: ALTYAPI PARAZİTİZMİ VE ENDÜKTİF HASAT</div>
        </div>
        <div class="meta-info">
    ...
    <div class="console-container">
        <div class="main-display">
            <div class="viz-view" id="viz-container">
                ...
                <!-- 85. Hücre: Infrastructure Parasitism Vizüalizasyonu -->
                <div class="viz-box" style="grid-column: span 6; grid-row: span 4;">
                    <div style="position:absolute; top:5px; left:10px; font-size:0.4rem; color:var(--infrastructure-parasitism); z-index:5;">MAGNETIC_FIELD_INDUCTIVE_HARVESTER</div>
                    <canvas id="parasite-canvas"></canvas>
                    <div id="parasite-hud" class="hud-text" style="color: var(--infrastructure-parasitism);">FIELD: 42.5µT | COUPLING: 94% | DRAIN: 1.2% | STATE: HARVESTING</div>
                </div>
            </div>
        ...
        <aside class="tools-panel">
            ...
            <button class="btn" id="parasite-toggle" onclick="InfrastructureParasitismEngine.toggle()">
                ...
                <span id="parasite-status">KAPALI</span>
            </button>
            ...
            <div style="margin-top: auto; font-size: 0.55rem; color: var(--text-dim); border-top: 1px solid var(--border); padding-top: 10px;">
                HÜCRE 85: Düşman altyapısından fiziksel temas olmadan veri ve enerji sızdırır.
            </div>
        </aside>
    </div>
    ...
    <footer class="protocol-footer">
        <div class="attribution">
            <p><b>85. HÜCRE: INFRASTRUCTURE PARASITISM</b> - Endüktif Sömürü Protokolü</p>
    ...
    <script type="module">
        const Kernel = {
            init() {
                ...
                this.log("Kernel aktif. Altyapı Parazitizmi (Parasitism) modülü mühürlendi.", "boot");
            },
            setupGrid() {
            ...
            async sequenceBoot() {
                const colors = ['active', 'boot-active', 'parasite-active'];
                for(let i=1; i<=85; i++) {
                ...
                this.log("85 Hücre endüktif sömürü ve parazit yüklemesi için senkronize.", "sys");
            }
        };
        ...
        const InfrastructureParasitismEngine = {
            ...
            toggle() {
                ...
                const statusText = document.getElementById('parasite-status');
                const hud = document.getElementById('parasite-hud');

                if(this.active) {
                    if(btn) btn.classList.add('active-op-parasite');
                    if(statusText) statusText.textContent = "EMİLİYOR";
                    Kernel.updateCell(85, "parasite-active");
                    Kernel.log("ALTYAPI PARAZİTİ AKTİF: MANYETİK ALAN HASADI BAŞLATILDI.", "parasite");
                    this.start();
                } else {
                    if(btn) btn.classList.remove('active-op-parasite');
                    if(statusText) statusText.textContent = "KAPALI";
                    Kernel.updateCell(85, "active");
                    cancelAnimationFrame(this.loop);
                    if(hud) hud.textContent = "FIELD: --µT | COUPLING: 0% | STATE: IDLE";
                    Kernel.log("Parazit emilim protokolü askıya alındı.", "sys");
                }
            },

            start() {
                const c = document.getElementById('parasite-canvas');
                if(!c) return;
                ...
                const draw = () => {
                    ...
                    const hud = document.getElementById('parasite-hud');
                    this.fieldStrength += (Math.random() - 0.5) * 2;
                    const coupling = 90 + Math.random() * 8;
                    const drain = (this.fieldStrength / 40).toFixed(2);
                    
                    if(hud) hud.textContent = `FIELD: ${this.fieldStrength.toFixed(1)}µT | COUPLING: ${Math.floor(coupling)}% | DRAIN: ${drain}% | STATE: HARVESTING`;
                    ...
                    for(let i=0; i<5; i++) {
                        ...
                        for(let x=0; x<c.width; x+=5) {
                            const y = (c.height / 2) + Math.sin(x * 0.02 + time * 5 + i) * (this.fieldStrength / 2);
                    ...
                    if(Math.random() > 0.8) {
                        this.waves.push({ x: Math.random() * c.width, y: c.height / 2, alpha: 1 });
                    }

                    this.waves.forEach((w_part, index) => {
                        w_part.y -= 2;
                        w_part.alpha -= 0.01;
                        if(w_part.alpha <= 0) this.waves.splice(index, 1);
                        
                        ctx.fillStyle = `rgba(74, 222, 128, ${w_part.alpha})`;
                        ctx.fillRect(w_part.x - 1, w_part.y - 1, 2, 2);
                        
                        if(Math.random() > 0.95) {
                            ctx.fillStyle = "white";
                            ctx.font = "6px monospace";
                            ctx.fillText(Math.random() > 0.5 ? "1" : "0", w_part.x + 5, w_part.y);
                        }
                    });

                    if(Math.random() > 0.99) {
                        Kernel.log(`Parasite: Düşman backbone hattından ${Math.floor(Math.random()*128)} bit veri paketlendi.`, "parasite");
                    }
        ...
        const NeuralLatticeEngine = {
            ...
            toggle() {
                ...
                Kernel.updateCell(84, this.active ? "lattice-active" : "active");
                if(this.active) { if(btn) btn.classList.add('active-op-lattice'); this.start(); } else { if(btn) btn.classList.remove('active-op-lattice'); cancelAnimationFrame(this.loop); }
            },
            start() {
                const c = document.getElementById('lattice-canvas'); if(!c) return;
                ...
                const draw = () => {
                    ...
                    ctx.strokeStyle = "var(--neural-lattice)"; ctx.strokeRect(c.width/2-20, c.height/2-20, 40, 40);
                    this.loop = requestAnimationFrame(draw);
                }; draw();
            }
        };
        ...
        const StaticMonitorEngine = {
            ...
            animate() {
                this.canvases.forEach(c_monitor => {
                    ...
                    if (c_monitor.id === 'lattice-canvas' && NeuralLatticeEngine.active) return;
