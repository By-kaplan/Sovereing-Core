...
<head>
    ...
    <title>NEFER-OS | 1.0.0-OMEGA | 81. HÜCRE (SOVEREIGN OS)</title>
    <style>
        :root {
            --bg-deep: #020617;
            --panel-bg: #0f172a;
            ...
            --sovereign-os: #f8fafc; /* 81. Hücre Rengi: Saf Beyaz / Egemenlik */
            --border: #1e293b;
            --cell-gap: 4px;
        }
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; cursor: crosshair; }
        ...
        .nefer-header {
            ...
            align-items: center;
            border-left: 4px solid var(--sovereign-os);
        }

        .nefer-header h1 { font-size: 1.2rem; color: var(--sovereign-os); letter-spacing: 3px; font-weight: 800; }
        ...
        .cell.sovereign-active { background: var(--sovereign-os); box-shadow: 0 0 20px var(--sovereign-os); animation: sovereign-flicker 0.5s infinite; }
        .cell.loading { background: var(--warning); animation: pulse 1s infinite; }

        @keyframes sovereign-flicker { 0%, 100% { opacity: 1; } 50% { opacity: 0.7; transform: scale(1.1); } }
        ...
        .viz-view {
            ...
            grid-template-rows: repeat(144, 35px) 1fr;
            gap: 2px;
            overflow-y: auto;
        }
        ...
        .log-entry.logi { border-left-color: var(--invisible-logistics); color: var(--invisible-logistics); }
        .log-entry.sovereign { border-left-color: var(--sovereign-os); color: var(--sovereign-os); font-weight: bold; background: rgba(248, 250, 252, 0.1); }
        ...
        .btn.active-op-sovereign { border-color: var(--sovereign-os); background: #f8fafc; color: #020617; border-width: 2px; }
...
<body>
    ...
    <header class="nefer-header">
        <div>
            <h1>NEFER-OS // SOVEREIGN_OS_SYNTHESIS</h1>
            <div style="font-size: 0.6rem; color: var(--text-dim);">81. HÜCRE: EGEMEN İŞLETİM SİSTEMİ SENTEZİ VE NİHAİ BEYİN</div>
        </div>
        <div class="meta-info">
    ...
    <div class="console-container">
        <div class="main-display">
            <div class="viz-view" id="viz-container">
                ...
                <!-- 81. Hücre: Sovereign OS Vizüalizasyonu -->
                <div class="viz-box" style="grid-column: span 6; grid-row: span 4;">
                    <div style="position:absolute; top:5px; left:10px; font-size:0.4rem; color:var(--sovereign-os); z-index:5;">KERNEL_LEVEL_RESOURCE_SCHEDULER</div>
                    <canvas id="sovereign-canvas"></canvas>
                    <div id="sovereign-hud" class="hud-text" style="color: var(--sovereign-os);">OS: NEFER-SYNTHESIS | CPU_RING: 0 | RAM_MAP: SEALED | STATE: SOVEREIGN</div>
                </div>
            </div>
        ...
        <aside class="tools-panel">
            ...
            <button class="btn" id="sovereign-toggle" onclick="SovereignOSEngine.toggle()">
                <span>81: SOVEREIGN OS SYNTHESIS</span>
                <span id="sovereign-status">KAPALI</span>
            </button>
            ...
            <div style="margin-top: auto; font-size: 0.55rem; color: var(--text-dim); border-top: 1px solid var(--border); padding-top: 10px;">
                HÜCRE 81: Donanım kaynaklarını doğrudan yöneterek sistemi egemen kılar.
            </div>
        </aside>
    </div>
    ...
    <footer class="protocol-footer">
        <div class="attribution">
            <p><b>81. HÜCRE: SOVEREIGN OS SYNTHESIS</b> - Egemen Sistem Protokolü</p>
    ...
    <script type="module">
        const Kernel = {
            init() {
                ...
                this.log("Kernel aktif. Egemen İşletim Sistemi (Sovereign OS) sentezlendi.", "boot");
            },
            setupGrid() {
            ...
            async sequenceBoot() {
                const colors = ['active', 'boot-active', 'sovereign-active'];
                for(let i=1; i<=81; i++) {
                ...
                this.log("81 Hücre tam sentez ve donanım egemenliği için senkronize.", "sys");
            }
        };

        const SovereignOSEngine = {
            active: false,
            loop: null,
            memoryMap: [],
            cycles: 0,
            ...
            toggle() {
                ...
                const btn = document.getElementById('sovereign-toggle');
                const statusText = document.getElementById('sovereign-status');
                const hud = document.getElementById('sovereign-hud');

                if(this.active) {
                    if(btn) btn.classList.add('active-op-sovereign');
                    if(statusText) statusText.textContent = "EGEMEN";
                    Kernel.updateCell(81, "sovereign-active");
                    Kernel.log("EGEMEN İŞLETİM SİSTEMİ AKTİF: DONANIM KAYNAKLARI JS-WASM ÇEKİRDEĞİNE DELEGELENDİ.", "sovereign");
                    this.start();
                } else {
                    if(btn) btn.classList.remove('active-op-sovereign');
                    if(statusText) statusText.textContent = "KAPALI";
                    Kernel.updateCell(81, "active");
                    cancelAnimationFrame(this.loop);
                    if(hud) hud.textContent = "OS: LEGACY | CPU_RING: 3 | RAM_MAP: UNPROTECTED | STATE: VULNERABLE";
                    Kernel.log("Egemen sentez durduruldu. Sistem eski işletim sistemi katmanlarına geri dönüyor.", "sys");
                }
            },

            start() {
                const c = document.getElementById('sovereign-canvas');
                if(!c) return;
                const ctx = c.getContext('2d');
                c.width = c.offsetWidth; c.height = c.offsetHeight;

                this.memoryMap = Array.from({length: 128}, () => ({
                    val: Math.random(),
                    usage: Math.random() > 0.5 ? 'SYSTEM' : 'NEFER'
                }));
                ...
                const draw = () => {
                    ...
                    const hud = document.getElementById('sovereign-hud');
                    this.cycles++;
                    if(hud) hud.textContent = `OS: NEFER-SYNTHESIS | CYCLES: ${this.cycles} | RING: 0 | STATE: SOVEREIGN_EXECUTION`;

                    // Bellek Haritası Vizüalizasyonu
                    const cols = 16;
                    const cellSize = c.width / cols;
                    const rows = Math.ceil(this.memoryMap.length / cols);
                    const cellHeight = c.height / rows;

                    this.memoryMap.forEach((m, i) => {
                        const x = (i % cols) * cellSize;
                        const y = Math.floor(i / cols) * cellHeight;
                        
                        m.val = (m.val + 0.01) % 1;
                        const colorVal = Math.floor(m.val * 255);
                        
                        if(m.usage === 'SYSTEM') {
                            ctx.fillStyle = `rgba(100, 116, 139, ${0.1 + m.val * 0.3})`;
                        } else {
                            ctx.fillStyle = `rgba(248, 250, 252, ${0.5 + m.val * 0.5})`;
                        }
                        
                        ctx.fillRect(x + 1, y + 1, cellSize - 2, cellHeight - 2);
                        
                        // "İşlem" parçacıkları
                        if(Math.random() > 0.95) {
                            ctx.fillStyle = "var(--text-neon)";
                            ctx.fillRect(x + cellSize/2 - 2, y + cellHeight/2 - 2, 4, 4);
                        }
                    });

                    // Kernel Seviyesi Akış Çizgileri
                    ctx.strokeStyle = "rgba(248, 250, 252, 0.1)";
                    ctx.beginPath();
                    for(let i=0; i<c.height; i+=20) {
                        ctx.moveTo(0, i);
                        ctx.lineTo(c.width, i + Math.sin(this.cycles/20)*10);
                    }
                    ctx.stroke();

                    if(this.cycles % 200 === 0) {
                        Kernel.log(`Sovereign: RAM sektörü 0x${Math.floor(Math.random()*0xFFFF).toString(16)} yeniden adreslendi ve mühürlendi.`, "sovereign");
                    }
        ...
        const InvisibleLogisticsEngine = {
            ...
            toggle() {
                ...
                Kernel.updateCell(80, this.active ? "logi-active" : "active");
                if(this.active) { if(btn) btn.classList.add('active-op-logi'); this.start(); } else { if(btn) btn.classList.remove('active-op-logi'); cancelAnimationFrame(this.loop); }
            },
            start() {
                const c = document.getElementById('logistics-canvas'); if(!c) return;
                ...
                const draw = () => {
                    ...
                    ctx.strokeStyle = "var(--invisible-logistics)"; ctx.strokeRect(c.width/2-20, c.height/2-20, 40, 40);
                    this.loop = requestAnimationFrame(draw);
                }; draw();
            }
        };
        ...
        const StaticMonitorEngine = {
            ...
            animate() {
                this.canvases.forEach(c => {
                    ...
                    if (c.id === 'sovereign-canvas' && SovereignOSEngine.active) return;
