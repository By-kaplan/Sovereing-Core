...
<head>
    ...
    <title>NEFER-OS | 1.0.0-OMEGA | 58. HÜCRE (BIO SPOOF)</title>
    <style>
        :root {
            --bg-deep: #020617;
            --panel-bg: #0f172a;
            ...
            --biometric-spoof: #22d3ee; /* 58. Hücre Rengi: Biyometrik Siyan */
            --border: #1e293b;
            --cell-gap: 4px;
        }
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; cursor: crosshair; }
        ...
        .nefer-header {
            ...
            align-items: center;
            border-left: 4px solid var(--biometric-spoof);
        }

        .nefer-header h1 { font-size: 1.2rem; color: var(--biometric-spoof); letter-spacing: 3px; font-weight: 800; }
        ...
        .viz-view {
            ...
            grid-template-rows: repeat(52, 35px) 1fr;
            gap: 2px;
            overflow-y: auto;
        }
        ...
        .log-entry.bio { border-left-color: var(--biometric-spoof); color: var(--biometric-spoof); font-weight: bold; }
...
<body>
    ...
    <header class="nefer-header">
        <div>
            <h1>NEFER-OS // BIOMETRIC_DATA_SPOOFING</h1>
            <div style="font-size: 0.6rem; color: var(--text-dim);">58. HÜCRE: BİYOMETRİK VERİ YANILTMA VE KİMLİK GASPI</div>
        </div>
        <div class="meta-info">
    ...
    <div class="console-container">
        <div class="main-display">
            <div class="viz-view" id="viz-container">
                ...
                <!-- 58. Hücre: Biometric Spoof Vizüalizasyonu -->
                <div class="viz-box" style="grid-column: span 6; grid-row: span 4;">
                    <div style="position:absolute; top:5px; left:10px; font-size:0.4rem; color:var(--biometric-spoof); z-index:5;">FACIAL_RECOGNITION_BYPASS_MESH</div>
                    <canvas id="bio-canvas"></canvas>
                    <div id="bio-hud" class="hud-text" style="color: var(--biometric-spoof);">MATCH: 99.8% | TARGET: AUTH_ADMIN | STATE: SPOOFING</div>
                </div>
            </div>
        ...
        <aside class="tools-panel">
            ...
            <button class="btn" id="bio-toggle" onclick="BiometricSpoofEngine.toggle()">
                ...
                <span id="bio-status">KAPALI</span>
            </button>
            ...
            <div style="margin-top: auto; font-size: 0.55rem; color: var(--text-dim); border-top: 1px solid var(--border); padding-top: 10px;">
                HÜCRE 58: Biyometrik verileri simüle ederek kimlik doğrulama sistemlerini bypass eder.
            </div>
        </aside>
    </div>
    ...
    <footer class="protocol-footer">
        <div class="attribution">
            <p><b>58. HÜCRE: BIOMETRIC DATA SPOOFING</b> - Kimlik Gasp Protokolü</p>
    ...
    <script type="module">
        const Kernel = {
            init() {
                ...
                this.log("Kernel aktif. Biyometrik Yanıltma modülü hazır.", "boot");
            },
            setupGrid() {
            ...
            async sequenceBoot() {
                const colors = ['active', 'boot-active', 'bio-active'];
                for(let i=1; i<=92; i++) {
                ...
                this.log("Tüm hücreler operasyonel senkronizasyona girdi.", "sys");
            }
        };
        ...
        const BiometricSpoofEngine = {
            ...
            toggle() {
                ...
                if(this.active) {
                    ...
                    Kernel.updateCell(58, "bio-active");
                    Kernel.log("BİYOMETRİK YANILTMA AKTİF: DERİN SAHTE (DEEPFAKE) ENJEKSİYONU BAŞLADI.", "bio");
                    this.start();
                } else {
                    if(btn) btn.classList.remove('active-op-bio');
                    if(statusText) statusText.textContent = "KAPALI";
                    Kernel.updateCell(58, "active");
                    cancelAnimationFrame(this.loop);
                    if(hud) hud.textContent = "MATCH: 0.0% | TARGET: NONE | STATE: IDLE";
                    Kernel.log("Biyometrik yanıltma protokolü durduruldu.", "sys");
                }
            },
            ...
            start() {
                ...
                // Yüz mesh noktaları simülasyonu
                this.points = Array.from({length: 15}, () => ({
                    x: c.width / 2, y: c.height / 2,
                    targetX: Math.random() * c.width,
                    targetY: Math.random() * c.height
                }));
                ...
                const draw = () => {
                    ...
                    const matchPercent = (95 + Math.random() * 4.9).toFixed(1);
                    const hud = document.getElementById('bio-hud');
                    if(hud) hud.textContent = `MATCH: ${matchPercent}% | TARGET: AUTH_ADMIN | STATE: INJECTING`;

                    ctx.strokeStyle = "var(--biometric-spoof)";
                    ctx.lineWidth = 1;

                    // Yüz ağı (Face Mesh) vizüalizasyonu
                    ctx.beginPath();
                    this.points.forEach((p, i) => {
                        ...
                        if(Math.abs(p.x - p.targetX) < 1) {
                            p.targetX = (c.width/2 - 50) + Math.random() * 100;
                            p.targetY = (c.height/2 - 70) + Math.random() * 140;
                        }
                    ...
                    // Tarama çizgisi
                    const scanY = (Date.now() / 10 % c.height);
                    ctx.strokeStyle = "rgba(34, 211, 238, 0.5)";
                    ctx.beginPath();
                    ctx.moveTo(0, scanY); ctx.lineTo(c.width, scanY);
                    ctx.stroke();

                    if(Math.random() > 0.98) {
                        Kernel.log(`BioSpoof: Yüz haritalama tamamlandı. Liveness-Test bypass edildi.`, "bio");
                    }
        ...
        const ThermalSabotageEngine = {
            ...
            start() {
                ...
                const draw = () => {
                    ...
                    if(hud) hud.textContent = `TEMP: ${this.temp.toFixed(1)}°C | FANS: 0 RPM | VCORE: 1.55V | STATE: MELTING`;
