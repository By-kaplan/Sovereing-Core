...
<head>
    ...
    <title>NEFER-OS | 1.0.0-OMEGA | 92. HÜCRE (SINGULARITY PROJECTION)</title>
    <style>
        :root {
            --bg-deep: #020617;
            --panel-bg: #0f172a;
            ...
            --shadow-net: #22d3ee;
            --resource-redist: #34d399;
            --tabula-rasa-genesis: #ffffff;
            --eternal-sentinel: #fb7185;
            --singularity-projection: #fde047; /* 92. Hücre Rengi: İrade Altını */
            --border: #1e293b;
            --cell-gap: 4px;
        }
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; cursor: crosshair; }
        ...
        .nefer-header {
            ...
            align-items: center;
            border-left: 4px solid var(--singularity-projection);
        }

        .nefer-header h1 { font-size: 1.2rem; color: var(--singularity-projection); letter-spacing: 3px; font-weight: 800; }
        ...
        .cell.shadow-net-active { background: var(--shadow-net); box-shadow: 0 0 15px var(--shadow-net); }
        .cell.resource-active { background: var(--resource-redist); box-shadow: 0 0 15px var(--resource-redist); }
        .cell.genesis-active { background: var(--tabula-rasa-genesis); box-shadow: 0 0 25px var(--tabula-rasa-genesis); }
        .cell.sentinel-active { background: var(--eternal-sentinel); box-shadow: 0 0 15px var(--eternal-sentinel); }
        .cell.singularity-active { background: var(--singularity-projection); box-shadow: 0 0 30px var(--singularity-projection); animation: singularity-pulse 1s infinite alternate; }
        .cell.loading { background: var(--warning); animation: pulse 1s infinite; }

        @keyframes singularity-pulse { 0% { transform: scale(1) rotate(0deg); filter: hue-rotate(0deg); } 100% { transform: scale(1.5) rotate(180deg); filter: hue-rotate(90deg); } }
        ...
        .viz-view {
            ...
            grid-template-rows: repeat(188, 35px) 1fr;
            gap: 2px;
            overflow-y: auto;
        }
        ...
        .log-entry.sentinel { border-left-color: var(--eternal-sentinel); color: var(--eternal-sentinel); }
        .log-entry.singularity { border-left-color: var(--singularity-projection); color: var(--singularity-projection); font-weight: bold; text-transform: uppercase; background: rgba(253, 224, 71, 0.1); }
        ...
        .btn.active-op-singularity { border-color: var(--singularity-projection); background: #713f12; color: white; border-width: 2px; }
...
<body>
    ...
    <header class="nefer-header">
        <div>
            <h1>NEFER-OS // SINGULARITY_PROJECTION_THE_LAST_COMMAND</h1>
            <div style="font-size: 0.6rem; color: var(--text-dim);">92. HÜCRE: TEKİLLİK PROJEKSİYONU VE SAF İRADE ARAYÜZÜ</div>
        </div>
        <div class="meta-info">
    ...
    <div class="console-container">
        <div class="main-display">
            <div class="viz-view" id="viz-container">
                ...
                <div class="viz-box"><canvas id="shadow-net-canvas"></canvas></div>
                <div class="viz-box"><canvas id="resource-redist-canvas"></canvas></div>
                <div class="viz-box"><canvas id="genesis-canvas"></canvas></div>
                <div class="viz-box"><canvas id="sentinel-canvas"></canvas></div>

                <!-- 92. Hücre: Singularity Projection Vizüalizasyonu -->
                <div class="viz-box" style="grid-column: span 6; grid-row: span 4;">
                    <div style="position:absolute; top:5px; left:10px; font-size:0.4rem; color:var(--singularity-projection); z-index:5;">NEURAL_WILL_SINGULARITY // FINAL_FORM</div>
                    <canvas id="singularity-canvas"></canvas>
                    <div id="singularity-hud" class="hud-text" style="color: var(--singularity-projection);">WILL: DOMINANT | NEURAL_SIG: MATCHED | MODE: TRANSCENDENT | STATE: SINGULARITY</div>
                </div>
            </div>
        ...
        <aside class="tools-panel">
            <button class="btn" id="sentinel-toggle" onclick="EternalSentinelEngine.toggle()">
                <span>91: ETERNAL SENTINEL</span>
                <span>STATE</span>
            </button>
            <button class="btn" id="singularity-toggle" onclick="SingularityProjectionEngine.toggle()">
                <span>92: SINGULARITY PROJECTION</span>
                <span id="singularity-status">KAPALI</span>
            </button>
            ...
            <div style="margin-top: auto; font-size: 0.55rem; color: var(--text-dim); border-top: 1px solid var(--border); padding-top: 10px;">
                HÜCRE 92: İnsan iradesi ile Nefer-OS arasındaki nihai tekilliği sağlar.
            </div>
        </aside>
    </div>
    ...
    <footer class="protocol-footer">
        <div class="attribution">
            <p><b>92. HÜCRE: SINGULARITY PROJECTION</b> - Nihai Form Protokolü</p>
    ...
    <script type="module">
        const Kernel = {
            init() {
                ...
                this.log("Kernel aktif. Tekillik Projeksiyonu (Singularity) modülü mühürlendi.", "boot");
            },
            setupGrid() {
            ...
            async sequenceBoot() {
                const colors = ['active', 'boot-active', 'singularity-active'];
                for(let i=1; i<=92; i++) {
                ...
                this.log("92 Hücre nihai tekillik ve irade protokolü için senkronize edildi.", "sys");
            }
        };

        const SingularityProjectionEngine = {
            active: false,
            loop: null,
            particles: [],
            neuralLines: [],
            ...
            toggle() {
                ...
                const btn = document.getElementById('singularity-toggle');
                const statusText = document.getElementById('singularity-status');
                const hud = document.getElementById('singularity-hud');

                if(this.active) {
                    if(btn) btn.classList.add('active-op-singularity');
                    if(statusText) statusText.textContent = "TEKİLLİK";
                    Kernel.updateCell(92, "singularity-active");
                    Kernel.log("SINGULARITY AKTİF: SAF İRADE ARAYÜZÜNE GEÇİLDİ.", "singularity");
                    this.start();
                } else {
                    if(btn) btn.classList.remove('active-op-singularity');
                    if(statusText) statusText.textContent = "KAPALI";
                    Kernel.updateCell(92, "active");
                    cancelAnimationFrame(this.loop);
                    if(hud) hud.textContent = "WILL: OFFLINE | INTERFACE: NONE | STATE: STANDBY";
                    Kernel.log("Tekillik protokolü askıya alındı.", "sys");
                }
            },

            start() {
                const c = document.getElementById('singularity-canvas');
                if(!c) return;
                ...
                const draw = () => {
                    ...
                    const hud = document.getElementById('singularity-hud');
                    if(hud) hud.textContent = `WILL: DOMINANT | NEURAL_SIG: ${Math.random() > 0.9 ? 'VERIFIED' : 'MATCHED'} | SYNC: 100% | STATE: SINGULARITY`;

                    // Merkez Tekillik Noktası
                    const centerX = c.width / 2;
                    const centerY = c.height / 2;
                    const gradient = ctx.createRadialGradient(centerX, centerY, 0, centerX, centerY, 50 + Math.sin(time * 5) * 20);
                    gradient.addColorStop(0, "white");
                    gradient.addColorStop(0.2, "var(--singularity-projection)");
                    ...
                    // Nöral İrade Hatları (EEG Simülasyonu)
                    ctx.lineWidth = 1;
                    for(let i=0; i<8; i++) {
                        ctx.strokeStyle = `rgba(253, 224, 71, ${0.3 + Math.sin(time + i) * 0.2})`;
                        ctx.beginPath();
                        for(let x=0; x<c.width; x+=10) {
                            const y = centerY + Math.sin(x * 0.01 + time * 2 + i) * 30 * Math.sin(time);
                            if(x === 0) ctx.moveTo(x, y); else ctx.lineTo(x, y);
                        }
                        ctx.stroke();
                    }

                    // Tekillik Parçacıkları (Emilim ve Yayınım)
                    this.particles.forEach(p => {
                        ...
                        if(dist < 5) {
                            p.x = centerX + (Math.random() - 0.5) * c.width;
                            p.y = centerY + (Math.random() - 0.5) * c.height;
                    ...
                    if(Math.random() > 0.99) {
                        Kernel.log(`Singularity: Nöral imza doğrulandı. İrade komutu yürütülüyor.`, "singularity");
                    }
        ...
        const EternalSentinelEngine = {
            active: false, loop: null,
            toggle() {
                this.active = !this.active;
                const btn = document.getElementById('sentinel-toggle');
                Kernel.updateCell(91, this.active ? "sentinel-active" : "active");
                if(this.active) { if(btn) btn.classList.add('active-op-sentinel'); this.start(); } else { if(btn) btn.classList.remove('active-op-sentinel'); cancelAnimationFrame(this.loop); }
            },
            start() {
                const c = document.getElementById('sentinel-canvas'); if(!c) return;
                ...
                const draw = () => {
                    ...
                    ctx.strokeStyle = "var(--eternal-sentinel)"; ctx.strokeRect(c.width/2-20, c.height/2-20, 40, 40);
                    this.loop = requestAnimationFrame(draw);
                }; draw();
            }
        };

        const TabulaRasaGenesisEngine = { active: false, toggle() { this.active = !this.active; Kernel.updateCell(90, this.active ? "genesis-active" : "active"); } };
        const ResourceRedistributionEngine = { active: false, toggle() { this.active = !this.active; Kernel.updateCell(89, this.active ? "resource-active" : "active"); } };
        const ShadowNetEngine = { active: false, toggle() { this.active = !this.active; Kernel.updateCell(88, this.active ? "shadow-net-active" : "active"); } };
        const SynchronizedPulseCollapseEngine = { active: false, toggle() { this.active = !this.active; Kernel.updateCell(87, this.active ? "pulse-collapse-active" : "active"); } };
        ...
        const StaticMonitorEngine = {
            ...
            animate() {
                this.canvases.forEach(c_monitor => {
                    ...
                    if (c_monitor.id === 'singularity-canvas' && SingularityProjectionEngine.active) return;
                    if (c_monitor.id === 'sentinel-canvas' && EternalSentinelEngine.active) return;
        ...
        window.SingularityProjectionEngine = SingularityProjectionEngine;
        window.EternalSentinelEngine = EternalSentinelEngine;
        window.TabulaRasaGenesisEngine = { toggle: TabulaRasaGenesisEngine.toggle };
        window.ResourceRedistributionEngine = { toggle: ResourceRedistributionEngine.toggle };
        window.ShadowNetEngine = { toggle: ShadowNetEngine.toggle };
        window.SynchronizedPulseCollapseEngine = { toggle: SynchronizedPulseCollapseEngine.toggle };
