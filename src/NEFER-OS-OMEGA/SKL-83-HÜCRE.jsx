...
<head>
    ...
    <title>NEFER-OS | 1.0.0-OMEGA | 83. HÜCRE (GLOBAL SYNC)</title>
    <style>
        :root {
            --bg-deep: #020617;
            --panel-bg: #0f172a;
            ...
            --sovereign-os: #f8fafc;
            --ai-fog: #a78bfa;
            --global-sync: #0ea5e9; /* 83. Hücre Rengi: İyonosfer Mavisi */
            --border: #1e293b;
            --cell-gap: 4px;
        }
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; cursor: crosshair; }
        ...
        .nefer-header {
            ...
            align-items: center;
            border-left: 4px solid var(--global-sync);
        }

        .nefer-header h1 { font-size: 1.2rem; color: var(--global-sync); letter-spacing: 3px; font-weight: 800; }
        ...
        .cell.sync-active { background: var(--global-sync); box-shadow: 0 0 15px var(--global-sync); animation: sync-wave-blink 2s infinite linear; }
        .cell.loading { background: var(--warning); animation: pulse 1s infinite; }

        @keyframes sync-wave-blink { 0% { opacity: 1; transform: scale(1); } 50% { opacity: 0.5; transform: scale(1.2); } 100% { opacity: 1; transform: scale(1); } }
        ...
        .viz-view {
            ...
            grid-template-rows: repeat(152, 35px) 1fr;
            gap: 2px;
            overflow-y: auto;
        }
        ...
        .log-entry.sync { border-left-color: var(--global-sync); color: var(--global-sync); font-weight: bold; background: rgba(14, 165, 233, 0.1); }
        ...
        .btn.active-op-sync { border-color: var(--global-sync); background: #0c4a6e; color: white; border-width: 2px; }
...
<body>
    ...
    <header class="nefer-header">
        <div>
            <h1>NEFER-OS // GLOBAL_MESH_SYNCHRONIZATION</h1>
            <div style="font-size: 0.6rem; color: var(--text-dim);">83. HÜCRE: KÜRESEL ÖRGÜ SENKRONİZASYONU VE VLF BEYNİ</div>
        </div>
        <div class="meta-info">
    ...
    <div class="console-container">
        <div class="main-display">
            <div class="viz-view" id="viz-container">
                ...
                <!-- 83. Hücre: Global Sync Vizüalizasyonu -->
                <div class="viz-box" style="grid-column: span 6; grid-row: span 4;">
                    <div style="position:absolute; top:5px; left:10px; font-size:0.4rem; color:var(--global-sync); z-index:5;">VLF_IONOSPHERIC_SYNC_MONITOR</div>
                    <canvas id="sync-canvas"></canvas>
                    <div id="sync-hud" class="hud-text" style="color: var(--global-sync);">MODE: VLF_RADIO | BITRATE: 12bps | IONO_REFLECT: 84% | STATE: SYNCING</div>
                </div>
            </div>
        ...
        <aside class="tools-panel">
            <button class="btn" id="fog-toggle" onclick="AICounterIntelligenceFogEngine.toggle()">
                <span>82: AI COUNTER-INTELLIGENCE FOG</span>
                <span>STATE</span>
            </button>
            <button class="btn" id="sync-toggle" onclick="GlobalMeshSyncEngine.toggle()">
                <span>83: GLOBAL MESH SYNCHRONIZATION</span>
                <span id="sync-status">KAPALI</span>
            </button>
            ...
            <div style="margin-top: auto; font-size: 0.55rem; color: var(--text-dim); border-top: 1px solid var(--border); padding-top: 10px;">
                HÜCRE 83: İnternet bağımsız, VLF dalgaları ile küresel sığınak senkronizasyonu sağlar.
            </div>
        </aside>
    </div>
    ...
    <footer class="protocol-footer">
        <div class="attribution">
            <p><b>83. HÜCRE: GLOBAL MESH SYNC</b> - Küresel Beyin Protokolü</p>
    ...
    <script type="module">
        const Kernel = {
            init() {
                ...
                this.log("Kernel aktif. Küresel Örgü Senkronizasyonu (Global Sync) hazır.", "boot");
            },
            setupGrid() {
            ...
            async sequenceBoot() {
                const colors = ['active', 'boot-active', 'sync-active'];
                for(let i=1; i<=83; i++) {
                ...
                this.log("83 Hücre küresel stratejik bütünlük için senkronize.", "sys");
            }
        };
        ...
        const GlobalMeshSyncEngine = {
            ...
            toggle() {
                ...
                if(this.active) {
                    if(btn) btn.classList.add('active-op-sync');
                    if(statusText) statusText.textContent = "SENKRON";
                    Kernel.updateCell(83, "sync-active");
                    Kernel.log("KÜRESEL SENKRONİZASYON AKTİF: VLF MODEMLERİ İYONOSFERE BAĞLANDI.", "sync");
                    this.start();
                } else {
                    if(btn) btn.classList.remove('active-op-sync');
                    if(statusText) statusText.textContent = "KAPALI";
                    Kernel.updateCell(83, "active");
                    cancelAnimationFrame(this.loop);
                    if(hud) hud.textContent = "MODE: OFFLINE | BITRATE: 0bps | STATE: STANDBY";
                    Kernel.log("Küresel senkronizasyon protokolü askıya alındı.", "sys");
                }
            },

            start() {
                const c = document.getElementById('sync-canvas');
                if(!c) return;
                ...
                const draw = () => {
                    ...
                    const hud = document.getElementById('sync-hud');
                    const reflect = 80 + Math.sin(Date.now()/2000)*10;
                    if(hud) hud.textContent = `MODE: VLF_RADIO | BITRATE: 12bps | IONO_REFLECT: ${Math.floor(reflect)}% | STATE: ${Math.random() > 0.1 ? 'SYNCING' : 'DATA_BURST'}`;

                    // VLF Dalga Efekti (İyonosfer yansıması)
                    ...
                    for(let i=0; i<3; i++) {
                        ...
                        for(let x=0; x<c.width; x+=10) {
                            ctx.lineTo(x, 20 + i*40 + Math.sin(x*0.02 + Date.now()/1000 + i)*20);
                        }
                        ctx.stroke();
                    }

                    // Sığınak Düğümleri ve Veri Paketleri
                    this.nodes.forEach((node, idx) => {
                        node.pulse += 0.05;
                        const s = 3 + Math.sin(node.pulse) * 2;
                        
                        ctx.fillStyle = node.active ? "var(--global-sync)" : "var(--hazard)";
                        ctx.beginPath();
                        ctx.arc(node.x, node.y, s, 0, Math.PI*2);
                        ctx.fill();
                        
                        if(node.active) {
                            ctx.strokeStyle = "var(--global-sync)";
                            ctx.strokeRect(node.x - 8, node.y - 8, 16, 16);
                            
                            // Rastgele veri sıçraması (Death Scream simülasyonu değil, normal akış)
                            if(Math.random() > 0.98) {
                                ctx.fillStyle = "white";
                                ctx.font = "6px monospace";
                                ctx.fillText("BURST_0x" + Math.floor(Math.random()*0xFF).toString(16), node.x + 10, node.y);
                            }
                        }
                    });

                    // Death Scream (Ölüm Çığlığı) Tetikleyicisi
                    if(Math.random() > 0.998) {
                        const target = Math.floor(Math.random() * this.nodes.length);
                        this.nodes[target].active = false;
                        Kernel.log(`CRITICAL: Sığınak Node_${target} düştü! ÖLÜM ÇIĞLIĞI (Death Scream) yayılıyor...`, "hazard");
                        setTimeout(() => this.nodes[target].active = true, 3000);
                    }
        ...
        const AICounterIntelligenceFogEngine = {
            active: false, loop: null, particles: [], scents: ["SMART_FRIDGE_EMU", "OLD_TRANSFORMER_NOISE", "BASE_STATION_ERR", "TRAFFIC_LIGHT_HUB", "GHOST_SHELTER_V2"], currentScent: 0,
            toggle() {
                this.active = !this.active;
                const btn = document.getElementById('fog-toggle');
                Kernel.updateCell(82, this.active ? "fog-active" : "active");
                if(this.active) { if(btn) btn.classList.add('active-op-fog'); this.start(); } else { if(btn) btn.classList.remove('active-op-fog'); cancelAnimationFrame(this.loop); }
            },
            start() {
                const c = document.getElementById('fog-canvas'); if(!c) return;
                ...
                const draw = () => {
                    ...
                    ctx.strokeStyle = "rgba(167, 139, 250, 0.3)"; ctx.beginPath(); ctx.arc(c.width/2, c.height/2, 40, 0, Math.PI*2); ctx.stroke();
                    this.loop = requestAnimationFrame(draw);
                }; draw();
            }
        };
