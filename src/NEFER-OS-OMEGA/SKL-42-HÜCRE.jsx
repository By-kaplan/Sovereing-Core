...
<head>
    ...
    <title>NEFER-OS | 1.0.0-OMEGA | 42. HÜCRE (DEAD-LOCK CASCADE)</title>
    <style>
        :root {
            --bg-deep: #020617;
            --panel-bg: #0f172a;
            ...
            --dead-lock-cascade: #4338ca; /* 42. Hücre Rengi: Kaskad İndigo */
            --border: #1e293b;
            --cell-gap: 4px;
        }
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; cursor: crosshair; }
        ...
        .nefer-header {
            ...
            align-items: center;
            border-left: 4px solid var(--dead-lock-cascade);
        }

        .nefer-header h1 { font-size: 1.2rem; color: var(--dead-lock-cascade); letter-spacing: 3px; font-weight: 800; }
        ...
        .viz-view {
            ...
            grid-template-rows: repeat(6, 70px) 1fr;
            gap: 2px;
        }
        ...
        .log-entry.heuristic { border-left-color: var(--heuristic-sabotage); color: var(--heuristic-sabotage); }
        .log-entry.cascade { border-left-color: var(--dead-lock-cascade); color: var(--dead-lock-cascade); font-weight: bold; }
...
<body>
    ...
    <header class="nefer-header">
        <div>
            <h1>NEFER-OS // DEAD_LOCK_CASCADE</h1>
            <div style="font-size: 0.6rem; color: var(--text-dim);">42. HÜCRE: DAĞITIK ÖLÜM-KİLİDİ KASKADI VE DAİRESEL BEKLEME</div>
        </div>
        <div class="meta-info">
    ...
    <div class="console-container">
        <div class="main-display">
            <div class="viz-view">
                ...
                <div class="viz-box" style="grid-column: span 3;">
                    <div style="position:absolute; top:5px; left:10px; font-size:0.4rem; color:var(--dead-lock-cascade); z-index:5;">DISTRIBUTED_LOCK_CHAIN</div>
                    <canvas id="cascade-canvas"></canvas>
                    <div id="cascade-hud" class="hud-text" style="color: var(--dead-lock-cascade);">NODES: 0 | WAIT: 0ms | STATE: STABLE</div>
                </div>
        ...
        <aside class="tools-panel">
            <button class="btn" onclick="HeuristicEngine.toggle()">
                <span>41: HEURISTIC PATTERN SABOTAGE</span>
                <span>DURUM</span>
            </button>
            <button class="btn" id="cascade-toggle" onclick="CascadeEngine.toggle()">
                <span>42: DISTRIBUTED DEAD-LOCK CASCADE</span>
                <span id="cascade-status">KAPALI</span>
            </button>
            ...
            <div style="margin-top: auto; font-size: 0.55rem; color: var(--text-dim); border-top: 1px solid var(--border); padding-top: 10px;">
                HÜCRE 42: Dairesel bekleme (Circular Wait) yaratarak ağ yönetimini felç eder.
            </div>
        </aside>
    </div>
    ...
    <footer class="protocol-footer">
        <div class="attribution">
            <p><b>42. HÜCRE: DISTRIBUTED DEAD-LOCK CASCADE</b> - Sistem Kilitleme Protokolü</p>
    ...
    <script type="module">
        const Kernel = {
            ...
            async sequenceBoot() {
                for(let i=1; i<=42; i++) {
                    if([1, 24, 41, 42].includes(i)) continue;
        ...
        const CascadeEngine = {
            ...
            toggle() {
                ...
                const hud = document.getElementById('cascade-hud');
                
                if(this.active) {
                    btn.classList.add('active-op-cascade');
                    document.getElementById('cascade-status').textContent = "KASKAD";
                    Kernel.updateCell(42, "cascade-active");
                    Kernel.log("ÖLÜM-KİLİDİ KASKADI AKTİF: DAİRESEL BEKLEME DÖNGÜSÜ KURULDU.", "cascade");
                    this.startViz();
                } else {
                    btn.classList.remove('active-op-cascade');
                    document.getElementById('cascade-status').textContent = "KAPALI";
                    Kernel.updateCell(42, "active");
                    cancelAnimationFrame(this.loop);
                    hud.textContent = "NODES: 0 | WAIT: 0ms | STATE: STABLE";
                    Kernel.log("Kaskad döngüsü pasifize edildi.", "sys");
                }
            },
            ...
            startViz() {
                ...
                const draw = () => {
                    ...
                    this.nodes = (this.nodes + 1) % 1000;
                    const waitTime = (Math.random() * 5000).toFixed(0);
                    document.getElementById('cascade-hud').textContent = `NODES: ${this.nodes} | WAIT: ${waitTime}ms | STATE: DEAD-LOCK`;

                    // Dairesel ağ düğümleri vizüalizasyonu
                    ...
                    for(let i=0; i<8; i++) {
                        ...
                        // Bağlantılar (Wait arrows)
                        const nextAngle = ((i + 1) / 8) * Math.PI * 2 + (Date.now() / 1000);
                    ...
                    if(Math.random() > 0.98) {
                        Kernel.log(`Cascade: Node_${this.nodes % 12} kaynağı SharedArrayBuffer üzerinden kilitlendi.`, "cascade");
                        Kernel.log(`Race Condition: Atomics.wait ile dairesel onay zinciri tetiklendi.`, "cascade");
                    }
        ...
        const HeuristicEngine = {
            ...
            toggle() {
                this.active = !this.active;
                Kernel.updateCell(41, this.active ? "heuristic-active" : "active");
