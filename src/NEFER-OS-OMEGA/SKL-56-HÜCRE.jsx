...
<head>
    ...
    <title>NEFER-OS | 1.0.0-OMEGA | 56. HÜCRE (REGISTRY ANCHOR)</title>
    <style>
        :root {
            --bg-deep: #020617;
            --panel-bg: #0f172a;
            ...
            --anti-sandbox: #6366f1;
            --zero-day: #84cc16;
            --registry-anchor: #94a3b8; /* 56. Hücre Rengi: Çelik Grisi */
            --border: #1e293b;
            --cell-gap: 4px;
        }
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; cursor: crosshair; }
        ...
        .nefer-header {
            ...
            align-items: center;
            border-left: 4px solid var(--registry-anchor);
        }

        .nefer-header h1 { font-size: 1.2rem; color: var(--registry-anchor); letter-spacing: 3px; font-weight: 800; }
        ...
        .cell.anchor-active { background: var(--registry-anchor); box-shadow: 0 0 15px var(--registry-anchor); animation: anchor-cloning 2s infinite ease-in-out; }
        .cell.loading { background: var(--warning); animation: pulse 1s infinite; }

        @keyframes anchor-cloning { 0% { transform: scale(1); filter: brightness(1); } 50% { transform: scale(0.8); filter: brightness(1.5); } 100% { transform: scale(1); filter: brightness(1); } }
        ...
        .viz-view {
            ...
            grid-template-rows: repeat(42, 35px) 1fr;
            gap: 2px;
            overflow-y: auto;
        }
        ...
        .log-entry.anchor { border-left-color: var(--registry-anchor); color: var(--registry-anchor); font-weight: bold; }
        ...
        .btn.active-op-anchor { border-color: var(--registry-anchor); background: #334155; color: white; }
...
<body>
    ...
    <header class="nefer-header">
        <div>
            <h1>NEFER-OS // REGISTRY_CONFIG_PERSISTENT_ANCHORING</h1>
            <div style="font-size: 0.6rem; color: var(--text-dim);">56. HÜCRE: KAYIT DEFTERİ ÇAPALAMA VE SİSTEM KALICILIĞI</div>
        </div>
        <div class="meta-info">
    ...
    <div class="console-container">
        <div class="main-display">
            <div class="viz-view" id="viz-container">
                ...
                <!-- 56. Hücre: Registry Anchor Vizüalizasyonu -->
                <div class="viz-box" style="grid-column: span 6; grid-row: span 4;">
                    <div style="position:absolute; top:5px; left:10px; font-size:0.4rem; color:var(--registry-anchor); z-index:5;">SYSTEM_DNA_PERSISTENCE_ENGINE</div>
                    <canvas id="anchor-canvas"></canvas>
                    <div id="anchor-hud" class="hud-text" style="color: var(--registry-anchor);">PERSISTENCE: ACTIVE | ANCHOR: AUDIO_DRV_SYS | DNA_STATE: CLONED</div>
                </div>
            </div>
        ...
        <aside class="tools-panel">
            ...
            <button class="btn" id="anchor-toggle" onclick="RegistryAnchorEngine.toggle()">
                ...
                <span id="anchor-status">KAPALI</span>
            </button>
            ...
            <div style="margin-top: auto; font-size: 0.55rem; color: var(--text-dim); border-top: 1px solid var(--border); padding-top: 10px;">
                HÜCRE 56: Sistem kayıt defterine ve sürücü klasörlerine kalıcı çapalar atar.
            </div>
        </aside>
    </div>
    ...
    <footer class="protocol-footer">
        <div class="attribution">
            <p><b>56. HÜCRE: REGISTRY/CONFIG PERSISTENT ANCHORING</b> - Kalıcılık Protokolü</p>
    ...
    <script type="module">
        const Kernel = {
            init() {
                ...
                this.log("Kernel aktif. Kalıcılık (Persistence) modülü mühürlendi.", "boot");
            },
            setupGrid() {
            ...
            async sequenceBoot() {
                for(let i=1; i<=56; i++) {
                    if([1, 24, 55, 56].includes(i)) continue;
        ...
        const RegistryAnchorEngine = {
            ...
            toggle() {
                ...
                const statusText = document.getElementById('anchor-status');
                
                if(this.active) {
                    if(btn) btn.classList.add('active-op-anchor');
                    if(statusText) statusText.textContent = "ÇAPALANDI";
                    Kernel.updateCell(56, "anchor-active");
                    Kernel.log("KALICILIK AKTİF: KAYIT DEFTERİ VE SÜRÜCÜ KATMANINA ÇAPA ATILDI.", "anchor");
                    this.start();
                } else {
                    if(btn) btn.classList.remove('active-op-anchor');
                    if(statusText) statusText.textContent = "KAPALI";
                    Kernel.updateCell(56, "active");
                    cancelAnimationFrame(this.loop);
                    this.dnaState = 0;
                    if(hud) hud.textContent = "PERSISTENCE: ACTIVE | ANCHOR: AUDIO_DRV_SYS | DNA_STATE: CLONED";
                    Kernel.log("Kalıcılık protokolü askıya alındı (Görünürlük artıyor).", "sys");
                }
            },
            ...
            start() {
                ...
                const draw = () => {
                    ...
                    this.dnaState += 0.5;
                    const hud = document.getElementById('anchor-hud');
                    if(hud) hud.textContent = `PERSISTENCE: ACTIVE | CLONE_DEPTH: ${Math.floor(this.dnaState)} | DNA: INFECTED`;

                    // Çapalama Vizüalizasyonu (Disk yazma ve klonlama simülasyonu)
                    ...
                    for(let i=0; i<3; i++) {
                        ctx.beginPath();
                        const r = (this.dnaState % 100) + (i * 30);
                        ctx.arc(centerX, centerY, r % 150, 0, Math.PI * 2);
                        ctx.stroke();
                    }

                    // Sürücü ve Kayıt Defteri Simülasyonu
                    for(let i=0; i<40; i++) {
                        const x = Math.random() * c.width;
                        const y = Math.random() * c.height;
                        ctx.fillStyle = Math.random() > 0.9 ? "var(--hazard)" : "var(--registry-anchor)";
                        ctx.globalAlpha = 0.3;
                    ...
                    if(Math.random() > 0.985) {
                        const keys = ['HKLM\\Software\\Microsoft\\Windows\\CurrentVersion\\Run', 'C:\\Windows\\System32\\drivers\\AudioDrv.sys', 'HKCU\\Environment'];
                        const target = keys[Math.floor(Math.random() * keys.length)];
                        Kernel.log(`Registry: '${target}' noktasına kalıcı çapa yerleştirildi.`, "anchor");
                    }
        ...
        const ZeroDayEngine = {
            active: false, loop: null, leakPercent: 0,
            toggle() {
                this.active = !this.active;
                const btn = document.getElementById('zeroday-toggle');
                Kernel.updateCell(55, this.active ? "zeroday-active" : "active");
