...
<head>
    ...
    <title>NEFER-OS | 1.0.0-OMEGA | 60. HÜCRE (DEBT OVERLOAD)</title>
    <style>
        :root {
            --bg-deep: #020617;
            --panel-bg: #0f172a;
            ...
            --debt-overload: #f43f5e; /* 60. Hücre Rengi: Borç Kırmızısı */
            --border: #1e293b;
            --cell-gap: 4px;
        }
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; cursor: crosshair; }
        ...
        .nefer-header {
            ...
            align-items: center;
            border-left: 4px solid var(--debt-overload);
        }

        .nefer-header h1 { font-size: 1.2rem; color: var(--debt-overload); letter-spacing: 3px; font-weight: 800; }
        ...
        .cell.debt-active { background: var(--debt-overload); box-shadow: 0 0 15px var(--debt-overload); animation: debt-pulse 1s infinite alternate; }
        .cell.loading { background: var(--warning); animation: pulse 1s infinite; }

        @keyframes debt-pulse { 0% { filter: brightness(1); transform: scale(1); } 100% { filter: brightness(1.8); transform: scale(1.15); } }
        ...
        .viz-view {
            ...
            grid-template-rows: repeat(60, 35px) 1fr;
            gap: 2px;
            overflow-y: auto;
        }
        ...
        .log-entry.hallucination { border-left-color: var(--log-hallucination); color: var(--log-hallucination); }
        .log-entry.debt { border-left-color: var(--debt-overload); color: var(--debt-overload); font-weight: bold; }
...
<body>
    ...
    <header class="nefer-header">
        <div>
            <h1>NEFER-OS // COMPUTATIONAL_DEBT_OVERLOAD</h1>
            <div style="font-size: 0.6rem; color: var(--text-dim);">60. HÜCRE: HESAPLAMA BORCU YÜKLEMESİ VE EKONOMİK İFLAS</div>
        </div>
        <div class="meta-info">
    ...
    <div class="console-container">
        <div class="main-display">
            <div class="viz-view" id="viz-container">
                ...
                <!-- 60. Hücre: Computational Debt Vizüalizasyonu -->
                <div class="viz-box" style="grid-column: span 6; grid-row: span 4;">
                    <div style="position:absolute; top:5px; left:10px; font-size:0.4rem; color:var(--debt-overload); z-index:5;">SERIAL_COMPUTATION_DEBT_TRAP</div>
                    <canvas id="debt-canvas"></canvas>
                    <div id="debt-hud" class="hud-text" style="color: var(--debt-overload);">ENERGY_DRAIN: 0W | CPU_DEBT: 0! | SOLVABILITY: 1e-99%</div>
                </div>
            </div>
        ...
        <aside class="tools-panel">
            ...
            <button class="btn" id="debt-toggle" onclick="DebtOverloadEngine.toggle()">
                ...
                <span id="debt-status">KAPALI</span>
            </button>
            ...
            <div style="margin-top: auto; font-size: 0.55rem; color: var(--text-dim); border-top: 1px solid var(--border); padding-top: 10px;">
                HÜCRE 60: Düşman sistemlerine çözülmesi imkansız ve maliyetli matematiksel yükler bindirir.
            </div>
        </aside>
    </div>
    ...
    <footer class="protocol-footer">
        <div class="attribution">
            <p><b>60. HÜCRE: COMPUTATIONAL DEBT OVERLOAD</b> - Donanımsal Ve Ekonomik İflas</p>
    ...
    <script type="module">
        const Kernel = {
            init() {
                ...
                this.log("Kernel aktif. Hesaplama Borcu (Debt Overload) modülü mühürlendi.", "boot");
            },
            setupGrid() {
            ...
            async sequenceBoot() {
                const colors = ['active', 'boot-active', 'bio-active', 'log-hall-active', 'debt-active'];
                for(let i=1; i<=92; i++) {
                ...
                this.log("Hücresel senkronizasyon tamamlandı. Omega protokolü hazır.", "sys");
            }
        };
        ...
        const DebtOverloadEngine = {
            ...
            toggle() {
                ...
                const statusText = document.getElementById('debt-status');
                
                if(this.active) {
                    if(btn) btn.classList.add('active-op-debt');
                    if(statusText) statusText.textContent = "BORÇLANDIRIYOR";
                    Kernel.updateCell(60, "debt-active");
                    Kernel.log("HESAPLAMA BORCU AKTİF: O(n!) ZİNCİRLEME TUZAK ENJEKTE EDİLDİ.", "debt");
                    this.start();
                } else {
                    if(btn) btn.classList.remove('active-op-debt');
                    if(statusText) statusText.textContent = "KAPALI";
                    Kernel.updateCell(60, "active");
                    cancelAnimationFrame(this.loop);
                    this.energy = 0; this.complexity = 0;
                    if(hud) hud.textContent = "ENERGY_DRAIN: 0W | CPU_DEBT: 0! | SOLVABILITY: 1e-99%";
                    Kernel.log("Hesaplama borcu protokolü durduruldu.", "sys");
                }
            },
            ...
            start() {
                ...
                const draw = () => {
                    ...
                    const hud = document.getElementById('debt-hud');
                    if(hud) hud.textContent = `ENERGY_DRAIN: ${Math.floor(this.energy)}kWh | CPU_DEBT: ${this.complexity}! | SOLVABILITY: ${(1/Math.pow(10, this.complexity)).toExponential(2)}%`;

                    // Borç Zinciri Vizüalizasyonu (Seri bağımlı düğümler)
                    ...
                    for(let i=0; i<20; i++) {
                        const nextX = lastX + (c.width / 20);
                        const nextY = (c.height/2) + Math.sin(Date.now()/500 + i)*30;
                        ctx.moveTo(lastX, lastY);
                        ctx.lineTo(nextX, nextY);
                        ctx.strokeRect(nextX-2, nextY-2, 4, 4);
                        lastX = nextX; lastY = nextY;
                    }
                    ctx.stroke();

                    // Radyasyon/Enerji sızıntısı efekti
                    ctx.fillStyle = "rgba(244, 63, 94, 0.1)";
                    for(let i=0; i<5; i++) {
                        ctx.beginPath();
                        ctx.arc(Math.random()*c.width, Math.random()*c.height, Math.random()*40, 0, Math.PI*2);
                        ctx.fill();
                    }

                    if(Math.random() > 0.985) {
                        Kernel.log(`Debt: Düşman çekirdeği #${Math.floor(Math.random()*64)} %100 yükte kilitlendi.`, "debt");
                    }
        ...
        const LogHallucinationEngine = {
            ...
            start() {
                ...
                const draw = () => {
                    ...
                    if(Math.random() > 0.9) {
                        this.lines.unshift(`[${new Date().toLocaleTimeString()}] ALERT: INTERNAL_LEAK DETECTED`);
