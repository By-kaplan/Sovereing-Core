...
<head>
    ...
    <title>NEFER-OS | 1.0.0-OMEGA | 15. HÜCRE (GHOSTING)</title>
    <style>
        :root {
            --bg-deep: #020617;
            --panel-bg: #0f172a;
            ...
            --dissonance: #84cc16;
            --corruption: #dc2626;
            --ghosting: #facc15; /* 15. Hücre Rengi: Hayalet Sarısı */
            --border: #1e293b;
            --cell-gap: 4px;
        }
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; cursor: crosshair; }
        ...
        .nefer-header {
            ...
            align-items: center;
            border-left: 4px solid var(--ghosting);
        }

        .nefer-header h1 { font-size: 1.2rem; color: var(--ghosting); letter-spacing: 3px; font-weight: 800; }
...
<body>
    ...
    <header class="nefer-header">
        <div>
            <h1>NEFER-OS // ALGORITHMIC_GHOSTING</h1>
            <div style="font-size: 0.6rem; color: var(--text-dim);">15. HÜCRE: ALGORİTMİK AYNALAMA VE ÖZ-SAVUNMA SABOTAJI</div>
        </div>
        <div class="meta-info">
    ...
    <div class="console-container">
        <div class="main-display">
            <div class="viz-view">
                <div class="viz-box" id="corrupt-viz"><canvas id="corrupt-canvas"></canvas></div>
                <div class="viz-box" id="ghost-viz">
                    <div style="position:absolute; top:5px; left:10px; font-size:0.4rem; color:var(--ghosting); background:rgba(0,0,0,0.8); z-index:5;">IDS_MIRROR_PROTOCOL_ACTIVE</div>
                    <canvas id="ghost-canvas"></canvas>
                    <div id="ghost-hud" class="ghost-hud">MIRROR_RATIO: 0% | TARGET: INTRANET</div>
                </div>
            </div>
        ...
        <aside class="tools-panel">
            <button class="btn" onclick="CorruptionEngine.toggle()"><span>14: RECURSIVE CORRUPTION</span></button>
            <button class="btn" id="ghost-toggle" onclick="GhostEngine.toggle()">
                <span>15: ALGORITHMIC GHOSTING</span>
                <span id="ghost-status">KAPALI</span>
            </button>
            ...
            <div style="margin-top: auto; font-size: 0.55rem; color: var(--text-dim); border-top: 1px solid var(--border); padding-top: 10px;">
                HÜCRE 15: Düşman IDS'ini kendi ana sunucusuna saldırmaya zorlayan aynalama motoru.
            </div>
        </aside>
    </div>
    ...
    <footer class="protocol-footer">
        <div class="attribution">
            <p><b>15. HÜCRE: ALGORITHMIC MIRRORING & GHOSTING</b> - IDS Sabotajı</p>
    ...
    <script>
        const Kernel = {
            ...
            init() {
                ...
                this.log("Kernel aktif. Hücre 15 aynalama katmanı senkronize edildi.", "sys");
            },
            setupGrid() {
        ...
        const CorruptionEngine = {
            ...
            toggle() {
                ...
                Kernel.updateCell(14, this.active ? "corruption-active" : "active");
            }
        };
        ...
        const GhostEngine = {
            ...
            loop: null,
            mirrorCount: 0,
            intranetPool: ["10.0.0.1", "192.168.1.50", "172.16.0.100"],
            ...
            toggle() {
                ...
                if(this.active) {
                    btn.classList.add('active-op-ghost');
                    document.getElementById('ghost-status').textContent = "AKTİF";
                    Kernel.updateCell(15, "ghosting-active");
                    Kernel.log("AYNALAMA PROTOKOLÜ BAŞLATILDI: IDS KENDİ TRAFİĞİNİ HEDEFLİYOR.", "ghost");
                    this.start(canvas, ctx, hud);
                } else {
                    btn.classList.remove('active-op-ghost');
                    document.getElementById('ghost-status').textContent = "KAPALI";
                    Kernel.updateCell(15, "active");
                    this.stop(ctx, canvas);
                    hud.textContent = "MIRROR_RATIO: 0% | TARGET: INTRANET";
                }
            },
            ...
            start(c, ctx, hud) {
                const mirror = () => {
                    ...
                    // Aynalama vizüalizasyonu: Simetrik dalgalar
                    const time = Date.now() / 200;
                    ctx.strokeStyle = "var(--ghosting)";
                    ctx.lineWidth = 1;
                    
                    ctx.beginPath();
                    for(let x=0; x<c.width; x++) {
                        const y = Math.sin(x/10 + time) * 20 + c.height/2;
                        const my = Math.sin(x/10 + time + Math.PI) * 20 + c.height/2; // Karşıt dalga
                        ctx.moveTo(x, y);
                        ctx.lineTo(x, my);
                    }
                    ctx.stroke();

                    this.mirrorCount = (this.mirrorCount + 1) % 100;

                    if(Math.random() > 0.98) {
                        const source = this.intranetPool[Math.floor(Math.random()*this.intranetPool.length)];
                        Kernel.log(`Mirror: Taklit edilen kaynak IP: ${source} (IDS Tetiklendi)`, "ghost");
                        Kernel.log(`Sabotaj: Düşman IDS kendi ana sunucusunu karantinaya aldı.`, "ghost");
                    }

                    hud.textContent = `MIRROR_RATIO: ${this.mirrorCount}% | SPOOF: ${this.intranetPool[0]}`;
                    
                    this.loop = requestAnimationFrame(mirror);
                };
                mirror();
            },
            ...
            stop(ctx, c) {
                ...
                Kernel.log("Aynalama durduruldu. Hayalet paketler silindi.", "sys");
            }
        };

        window.onload = () => {
            Kernel.init();
            ["corrupt", "ghost"].forEach(id => {
