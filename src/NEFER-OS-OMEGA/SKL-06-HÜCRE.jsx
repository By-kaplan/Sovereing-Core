...
<head>
    ...
    <title>NEFER-OS | 1.0.0-OMEGA | 06. HÜCRE (PROTOCOL SHADOWING)</title>
    <style>
        :root {
            --bg-deep: #020617;
            --panel-bg: #0f172a;
            ...
            --signal: #22d3ee; 
            --biometric: #10b981; 
            --shadow: #f43f5e; /* 06. Hücre Rengi: Gölge Kırmızısı */
            --border: #1e293b;
            --cell-gap: 4px;
        }
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; cursor: crosshair; }
        ...
        .nefer-header h1 { font-size: 1.2rem; color: var(--shadow); letter-spacing: 3px; font-weight: 800; }
        ...
        .log-entry.shadow { border-left-color: var(--shadow); color: var(--shadow); }
        ...
        .btn.active-op-shadow { border-color: var(--shadow); background: #4c0519; color: white; }
...
<body>
    ...
    <header class="nefer-header">
        <div>
            <h1>NEFER-OS // PROTOCOL_SHADOW</h1>
            <div style="font-size: 0.6rem; color: var(--text-dim);">06. HÜCRE: TRAFİK MASKELENME VE BEYAZ LİSTE TAKLİDİ</div>
        </div>
        <div class="meta-info">
    ...
    <div class="console-container">
        <div class="main-display">
            <div class="viz-view">
                ...
                <div class="viz-box" id="shadow-viz">
                    <div style="position:absolute; top:5px; left:10px; font-size:0.4rem; color:var(--shadow); background:rgba(0,0,0,0.8); z-index:5;">WHITELIST_MIMICRY_ACTIVE</div>
        ...
        <aside class="tools-panel">
            <button class="btn" onclick="OpticEngine.toggle()"><span>02: OPTIC PARALYSIS</span></button>
            <button class="btn" onclick="EntropicEngine.toggle()"><span>03: ENTROPIC BACKFIRE</span></button>
            <button class="btn" onclick="SignalEngine.toggle()"><span>04: SIGNAL ECHO</span></button>
            <button class="btn" onclick="BiometricEngine.toggle()"><span>05: BIOMETRIC NOISE</span></button>
            <button class="btn" id="shadow-toggle" onclick="ShadowEngine.toggle()">
                <span>06: PROTOCOL SHADOW</span>
    ...
    <footer class="protocol-footer">
        <div class="attribution">
            <p><b>06. HÜCRE: PROTOCOL SHADOWING</b> - Beyaz Liste Protokol Maskesi</p>
    ...
    <script>
        const Kernel = {
            ...
            init() {
                ...
                this.log("Kernel aktif. Hücre 06 operasyonel.", "sys");
            },
            setupGrid() {
            ...
            async sequenceBoot() {
                for(let i=2; i<=92; i++) {
                    if([2,3,4,5,6].includes(i)) continue;
        ...
        const ShadowEngine = {
            ...
            toggle() {
                ...
                if(this.active) {
                    ...
                    document.getElementById('shadow-status').textContent = "AKTİF";
                    Kernel.updateCell(6, "shadow-active");
                    Kernel.log("TRAFİK GÖLGELEME AKTİF: CISCO/JUNIPER MASKESİ UYGULANIYOR.", "shadow");
                    this.start(canvas, ctx);
                } else {
                    btn.classList.remove('active-op-shadow');
                    document.getElementById('shadow-status').textContent = "KAPALI";
                    Kernel.updateCell(6, "active");
            ...
            start(c, ctx) {
                const draw = () => {
                    ...
                    for(let i=0; i<5; i++) {
                        ...
                        ctx.fillRect(0, y, c.width * Math.random(), 1);
                    }
                    
                    if(Math.random() > 0.95) {
                        const protocols = ["HTTPS_UPDATE", "WEATHER_SYNC", "OS_HEARTBEAT", "NTP_SYNC"];
                        const p = protocols[Math.floor(Math.random()*protocols.length)];
                        Kernel.log(`Paket Maskelendi: ${p} (DPI Bypass)`, "shadow");
                    }
