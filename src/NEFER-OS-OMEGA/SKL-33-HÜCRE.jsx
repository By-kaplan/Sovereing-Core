...
<head>
    ...
    <title>NEFER-OS | 1.0.0-OMEGA | 33. HÜCRE (TIME BOMB)</title>
    <style>
        :root {
            --bg-deep: #020617;
            --panel-bg: #0f172a;
            ...
            --neural-jam: #c084fc;
            --heap-spray: #f472b6;
            --time-bomb: #f59e0b; /* 33. Hücre Rengi: Bomba Kehribarı */
            --border: #1e293b;
            --cell-gap: 4px;
        }
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; cursor: crosshair; }
        ...
        .nefer-header {
            ...
            align-items: center;
            border-left: 4px solid var(--time-bomb);
        }

        .nefer-header h1 { font-size: 1.2rem; color: var(--time-bomb); letter-spacing: 3px; font-weight: 800; }
        ...
        .cell.bomb-active { background: var(--time-bomb); box-shadow: 0 0 15px var(--time-bomb); animation: bomb-flicker 0.4s infinite; }
        .cell.loading { background: var(--warning); animation: pulse 1s infinite; }

        @keyframes bomb-flicker { 0% { opacity: 1; filter: brightness(1); } 50% { opacity: 0.5; filter: brightness(2); } 100% { opacity: 1; filter: brightness(1); } }
        ...
        .log-entry.bomb { border-left-color: var(--time-bomb); color: var(--time-bomb); font-weight: bold; }
        ...
        .btn.active-op-bomb { border-color: var(--time-bomb); background: #78350f; color: white; }
...
<body>
    ...
    <header class="nefer-header">
        <div>
            <h1>NEFER-OS // TIME_SYNC_LOGIC_BOMB</h1>
            <div style="font-size: 0.6rem; color: var(--text-dim);">33. HÜCRE: ZAMAN-SENKRONLU MANTIK BOMBASI VE KÜRESEL TETİKLEME</div>
        </div>
        <div class="meta-info">
    ...
    <div class="console-container">
        <div class="main-display">
            <div class="viz-view">
                ...
                <div class="viz-box">
                    <div style="position:absolute; top:5px; left:10px; font-size:0.4rem; color:var(--time-bomb); z-index:5;">ATOMIC_CLOCK_SYNC</div>
                    <canvas id="bomb-canvas"></canvas>
                    <div id="bomb-hud" class="hud-text" style="color: var(--time-bomb);">TARGET: 2026-05-01 | SYNC: 0.00ms</div>
                </div>
                <div class="viz-box" style="grid-column: span 6;">
        ...
        <aside class="tools-panel">
            <button class="btn" onclick="HeapEngine.toggle()">
                <span>32: HEAP-SPRAY MEMORY DIVERSION</span>
                <span>STATE</span>
            </button>
            <button class="btn" id="bomb-toggle" onclick="BombEngine.toggle()">
                <span>33: TIME-SYNC LOGIC BOMB</span>
                <span id="bomb-status">KAPALI</span>
            </button>
            ...
            <div style="margin-top: auto; font-size: 0.55rem; color: var(--text-dim); border-top: 1px solid var(--border); padding-top: 10px;">
                HÜCRE 33: Atomik saat hassasiyetinde tüm uyuyan hücreleri aynı anda saldırıya geçirir.
            </div>
        </aside>
    </div>
    ...
    <footer class="protocol-footer">
        <div class="attribution">
            <p><b>33. HÜCRE: TIME-SYNC LOGIC BOMB</b> - Küresel Koordinasyon</p>
    ...
    <script type="module">
        const Kernel = {
            ...
            async sequenceBoot() {
                for(let i=1; i<=33; i++) {
                    if([1, 24, 28, 29, 30, 31, 32, 33].includes(i)) continue;
        ...
        const BombEngine = {
            active: false,
            loop: null,
            targetTime: new Date("2026-05-01T00:00:00").getTime(),
            
            toggle() {
                this.active = !this.active;
                const btn = document.getElementById('bomb-toggle');
                const hud = document.getElementById('bomb-hud');
                
                if(this.active) {
                    btn.classList.add('active-op-bomb');
                    document.getElementById('bomb-status').textContent = "SENKRONİZE";
                    Kernel.updateCell(33, "bomb-active");
                    Kernel.log("MANTIK BOMBASI KURULDU: ATOMİK SAAT SENKRONİZASYONU BAŞLADI.", "bomb");
                    this.startViz();
                } else {
                    btn.classList.remove('active-op-bomb');
                    document.getElementById('bomb-status').textContent = "KAPALI";
                    Kernel.updateCell(33, "active");
                    cancelAnimationFrame(this.loop);
                    hud.textContent = "TARGET: 2026-05-01 | SYNC: 0.00ms";
                    Kernel.log("Zaman ayarlı bomba pasifize edildi.", "sys");
                }
            },
            ...
            startViz() {
                ...
                const draw = () => {
                    ...
                    const syncDrift = (Math.random() * 0.05).toFixed(2);
                    
                    document.getElementById('bomb-hud').textContent = `SYNC: ${syncDrift}ms | T-MINUS: ${Math.floor(timeLeft/1000)}s`;

                    // Senkronizasyon halkası
                    ...
                    if(Math.random() > 0.99) {
                        Kernel.log(`Sync: Tüm "Uykuda" (Dormant) hücreler zaman damgasını doğruladı.`, "bomb");
                        Kernel.log(`Tetikleyici: 33. Hücre, atomik saat ile milisaniyelik uyum sağladı.`, "bomb");
                    }
