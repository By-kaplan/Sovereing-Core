...
<head>
    ...
    <title>NEFER-OS | 1.0.0-OMEGA | 40. HÜCRE (MUTATION KEYING)</title>
    <style>
        :root {
            --bg-deep: #020617;
            --panel-bg: #0f172a;
            ...
            --jit-exploit: #f87171;
            --stegano: #4ade80;
            --mutation-key: #ec4899; /* 40. Hücre Rengi: Mutasyon Pembesi */
            --border: #1e293b;
            --cell-gap: 4px;
        }
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; cursor: crosshair; }
        ...
        .nefer-header {
            ...
            align-items: center;
            border-left: 4px solid var(--mutation-key);
        }

        .nefer-header h1 { font-size: 1.2rem; color: var(--mutation-key); letter-spacing: 3px; font-weight: 800; }
        ...
        .cell.stegano-active { background: var(--stegano); box-shadow: 0 0 15px var(--stegano); }
        .cell.mutation-active { background: var(--mutation-key); box-shadow: 0 0 15px var(--mutation-key); animation: key-glitch 0.3s infinite; }
        .cell.loading { background: var(--warning); animation: pulse 1s infinite; }

        @keyframes key-glitch { 0% { filter: hue-rotate(0deg); } 50% { filter: hue-rotate(90deg); } 100% { filter: hue-rotate(0deg); } }
        ...
        .viz-view {
            ...
            grid-template-rows: repeat(4, 70px) 1fr;
            gap: 2px;
        }
        ...
        .log-entry.stegano { border-left-color: var(--stegano); color: var(--stegano); }
        .log-entry.mutation { border-left-color: var(--mutation-key); color: var(--mutation-key); font-weight: bold; }
...
<body>
    ...
    <header class="nefer-header">
        <div>
            <h1>NEFER-OS // CRYPTO_MUTATION_KEYING</h1>
            <div style="font-size: 0.6rem; color: var(--text-dim);">40. HÜCRE: KRİPTOGRAFİK MUTASYON ANAHTARLAMA VE OTP TÜRETİMİ</div>
        </div>
        <div class="meta-info">
    ...
    <div class="console-container">
        <div class="main-display">
            <div class="viz-view">
                ...
                <div class="viz-box">
                    <div style="position:absolute; top:5px; left:10px; font-size:0.4rem; color:var(--mutation-key); z-index:5;">CRYPTO_ENTROPY_GENERATOR</div>
                    <canvas id="mutation-canvas"></canvas>
                    <div id="mutation-hud" class="hud-text" style="color: var(--mutation-key);">KEY_GEN: PBKDF2 | ENTROPY: 0.0 | STATUS: STANDBY</div>
                </div>
        ...
        <aside class="tools-panel">
            <button class="btn" onclick="SteganoEngine.toggle()">
                <span>39: VISUAL STEGANOGRAPHIC OVERLAY</span>
                <span>DURUM</span>
            </button>
            <button class="btn" id="mutation-toggle" onclick="MutationKeyEngine.toggle()">
                <span>40: CRYPTOGRAPHIC MUTATION KEYING</span>
                <span id="mutation-status">KAPALI</span>
            </button>
            ...
            <div style="margin-top: auto; font-size: 0.55rem; color: var(--text-dim); border-top: 1px solid var(--border); padding-top: 10px;">
                HÜCRE 40: Şifreleme anahtarını sistem entropisinden dinamik olarak türetip anında yok eder.
            </div>
        </aside>
    </div>
    ...
    <footer class="protocol-footer">
        <div class="attribution">
            <p><b>40. HÜCRE: CRYPTOGRAPHIC MUTATION KEYING</b> - Dinamik OTP Protokolü</p>
    ...
    <script type="module">
        const Kernel = {
            ...
            async sequenceBoot() {
                for(let i=1; i<=40; i++) {
                    if([1, 24, 39, 40].includes(i)) continue;
        ...
        const MutationKeyEngine = {
            ...
            toggle() {
                ...
                const btn = document.getElementById('mutation-toggle');
                const hud = document.getElementById('mutation-hud');
                
                if(this.active) {
                    btn.classList.add('active-op-mutation');
                    document.getElementById('mutation-status').textContent = "DEĞİŞKEN";
                    Kernel.updateCell(40, "mutation-active");
                    Kernel.log("KRİPTOGRAFİK MUTASYON AKTİF: ANAHTAR SİSTEM ENTROPİSİNDEN TÜRETİLİYOR.", "mutation");
                    this.startViz();
                } else {
                    btn.classList.remove('active-op-mutation');
                    document.getElementById('mutation-status').textContent = "KAPALI";
                    Kernel.updateCell(40, "active");
                    cancelAnimationFrame(this.loop);
                    hud.textContent = "KEY_GEN: PBKDF2 | ENTROPY: 0.0 | STATUS: STANDBY";
                    Kernel.log("Mutasyon anahtarlama protokolü durduruldu.", "sys");
                }
            },
            ...
            startViz() {
                ...
                const draw = () => {
                    ...
                    // Simüle edilmiş entropi toplama
                    this.entropy = (performance.now() % 100) / 10;
                    
                    ctx.fillStyle = "rgba(2, 6, 23, 0.4)";
                    ctx.fillRect(0, 0, c.width, c.height);
                    
                    // Kripto dalga görselleştirmesi
                    ctx.strokeStyle = "var(--mutation-key)";
                    ctx.lineWidth = 1;
                    ctx.beginPath();
                    for(let i=0; i<c.width; i+=2) {
                        const y = (c.height/2) + Math.sin(i / 10 + Date.now() / 100) * (this.entropy * 2);
                        if(i===0) ctx.moveTo(i, y); else ctx.lineTo(i, y);
                    }
                    ctx.stroke();

                    // HUD Güncelleme
                    document.getElementById('mutation-hud').textContent = `KEY: 0x${(Math.random()*0xFFFFFF << 0).toString(16)} | ENTROPY: ${this.entropy.toFixed(2)} | OTP: READY`;

                    if(Math.random() > 0.98) {
                        Kernel.log(`Key Gen: PBKDF2 üzerinden yeni OTP anahtarı türetildi. [Source: performance.now()]`, "mutation");
                        Kernel.log(`Security: Eski anahtar bellekten (RAM) kalıcı olarak silindi.`, "mutation");
                    }
