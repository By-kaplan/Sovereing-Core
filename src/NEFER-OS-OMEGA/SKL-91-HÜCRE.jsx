...
<head>
    ...
    <title>NEFER-OS | 1.0.0-OMEGA | 91. HÜCRE (ETERNAL SENTINEL)</title>
    <style>
        :root {
            --bg-deep: #020617;
            --panel-bg: #0f172a;
            ...
            --eternal-sentinel: #fb7185; /* 91. Hücre Rengi: Nöbet Gül Pembesi */
            --border: #1e293b;
            --cell-gap: 4px;
        }
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; cursor: crosshair; }
        ...
        .nefer-header {
            ...
            align-items: center;
            border-left: 4px solid var(--eternal-sentinel);
        }

        .nefer-header h1 { font-size: 1.2rem; color: var(--eternal-sentinel); letter-spacing: 3px; font-weight: 800; }
        ...
        .cell.sentinel-active { background: var(--eternal-sentinel); box-shadow: 0 0 15px var(--eternal-sentinel); animation: sentinel-glow 2s infinite ease-in-out; }
        .cell.loading { background: var(--warning); animation: pulse 1s infinite; }

        @keyframes sentinel-glow { 0%, 100% { filter: brightness(1); transform: scale(1); } 50% { filter: brightness(1.8); transform: scale(1.1); } }
        ...
        .viz-view {
            ...
            grid-template-rows: repeat(184, 35px) 1fr;
            gap: 2px;
            overflow-y: auto;
        }
        ...
        .log-entry.genesis { border-left-color: var(--tabula-rasa-genesis); color: var(--tabula-rasa-genesis); }
        .log-entry.sentinel { border-left-color: var(--eternal-sentinel); color: var(--eternal-sentinel); font-weight: bold; background: rgba(251, 113, 133, 0.1); }
        ...
        .btn.active-op-sentinel { border-color: var(--eternal-sentinel); background: #881337; color: white; border-width: 2px; }
...
<body>
    ...
    <header class="nefer-header">
        <div>
            <h1>NEFER-OS // ETERNAL_SENTINEL_PROTOCOL</h1>
            <div style="font-size: 0.6rem; color: var(--text-dim);">91. HÜCRE: EBEDİ NÖBET VE SEZGİSEL ÖZ-EVRİM</div>
        </div>
        <div class="meta-info">
    ...
    <div class="console-container">
        <div class="main-display">
            <div class="viz-view" id="viz-container">
                ...
                <!-- 91. Hücre: Eternal Sentinel Vizüalizasyonu -->
                <div class="viz-box" style="grid-column: span 6; grid-row: span 4;">
                    <div style="position:absolute; top:5px; left:10px; font-size:0.4rem; color:var(--eternal-sentinel); z-index:5;">HEURISTIC_SELF_EVOLVER // IMMUNITY_PROTOCOL</div>
                    <canvas id="sentinel-canvas"></canvas>
                    <div id="sentinel-hud" class="hud-text" style="color: var(--eternal-sentinel);">GENOME: POLYMORPHIC | SIMULATION: ACTIVE | MUTATIONS: 1.4K/S | STATE: EVOLVING</div>
                </div>
            </div>
        ...
        <aside class="tools-panel">
            <button class="btn" id="genesis-toggle" onclick="TabulaRasaGenesisEngine.toggle()">
                <span>90: TABULA RASA GENESIS</span>
                <span>STATE</span>
            </button>
            <button class="btn" id="sentinel-toggle" onclick="EternalSentinelEngine.toggle()">
                <span>91: ETERNAL SENTINEL</span>
                <span id="sentinel-status">KAPALI</span>
            </button>
            ...
            <div style="margin-top: auto; font-size: 0.55rem; color: var(--text-dim); border-top: 1px solid var(--border); padding-top: 10px;">
                HÜCRE 91: Sistemde periyodik mutasyonlar yaparak otonom bir bağışıklık sağlar.
            </div>
        </aside>
    </div>
    ...
    <footer class="protocol-footer">
        <div class="attribution">
            <p><b>91. HÜCRE: ETERNAL SENTINEL</b> - Ebedi Nöbet Sistemi</p>
    ...
    <script type="module">
        const Kernel = {
            init() {
                ...
                this.log("Kernel aktif. Ebedi Nöbet (Sentinel) modülü mühürlendi.", "boot");
            },
            setupGrid() {
            ...
            async sequenceBoot() {
                const colors = ['active', 'boot-active', 'sentinel-active'];
                for(let i=1; i<=91; i++) {
                ...
                this.log("91 Hücre bağışıklık ve öz-evrim protokolü için senkronize edildi.", "sys");
            }
        };
        ...
        const EternalSentinelEngine = {
            ...
            toggle() {
                ...
                const statusText = document.getElementById('sentinel-status');
                const hud = document.getElementById('sentinel-hud');

                if(this.active) {
                    if(btn) btn.classList.add('active-op-sentinel');
                    if(statusText) statusText.textContent = "EVRİLİYOR";
                    Kernel.updateCell(91, "sentinel-active");
                    Kernel.log("SENTINEL PROTOKOLÜ AKTİF: HEURISTIC ÖZ-EVRİM BAŞLATILDI.", "sentinel");
                    this.start();
                } else {
                    if(btn) btn.classList.remove('active-op-sentinel');
                    if(statusText) statusText.textContent = "KAPALI";
                    Kernel.updateCell(91, "active");
                    cancelAnimationFrame(this.loop);
                    if(hud) hud.textContent = "GENOME: STATIC | SIMULATION: OFF | STATE: STANDBY";
                    Kernel.log("Ebedi nöbet protokolü askıya alındı.", "sys");
                }
            },

            start() {
                const c = document.getElementById('sentinel-canvas');
                if(!c) return;
                const ctx = c.getContext('2d');
                c.width = c.offsetWidth; c.height = c.offsetHeight;

                this.dnaNodes = Array.from({length: 40}, (_, i) => ({
                    x: (c.width / 40) * i,
                    y: c.height / 2,
                    phase: i * 0.2
                }));
                ...
                const draw = () => {
                    ...
                    this.mutationCount += Math.floor(Math.random() * 5);
                    const hud = document.getElementById('sentinel-hud');
                    if(hud) hud.textContent = `GENOME: POLYMORPHIC | SIMULATION: ACTIVE | MUTATIONS: ${this.mutationCount} | STATE: EVOLVING`;

                    // Çift Sarmal DNA Yapısı (Mutasyon Simülasyonu)
                    ctx.lineWidth = 2;
                    this.dnaNodes.forEach((node, i) => {
                        const shift = Math.sin(time * 2 + node.phase) * 40;
                        const colorIdx = (Math.floor(time * 5 + i) % 360);
                        
                        // Birinci Sarmal
                        ctx.fillStyle = `hsla(${colorIdx}, 70%, 60%, 0.8)`;
                        ctx.beginPath();
                        ctx.arc(node.x, node.y + shift, 3, 0, Math.PI*2);
                        ctx.fill();

                        // İkinci Sarmal
                        ctx.fillStyle = `hsla(${(colorIdx + 180) % 360}, 70%, 60%, 0.8)`;
                        ctx.beginPath();
                        ctx.arc(node.x, node.y - shift, 3, 0, Math.PI*2);
                        ctx.fill();
                        ...
                        ctx.beginPath();
                        ctx.moveTo(node.x, node.y + shift);
                        ctx.lineTo(node.x, node.y - shift);
                        ctx.stroke();
                    });

                    // Tarama/Nöbet Efekti
                    ctx.strokeStyle = "var(--eternal-sentinel)";
                    ctx.setLineDash([5, 15]);
                    const scanLine = (Math.sin(time) * 0.5 + 0.5) * c.width;
                    ctx.beginPath();
                    ctx.moveTo(scanLine, 0);
                    ctx.lineTo(scanLine, c.height);
                    ctx.stroke();
                    ctx.setLineDash([]);

                    if(Math.random() > 0.995) {
                        Kernel.log(`Sentinel: ${Math.floor(Math.random()*100)} potansiyel zafiyet tarandı ve polimorfik refaktör ile kapatıldı.`, "sentinel");
                    }
        ...
        const TabulaRasaGenesisEngine = {
            active: false, loop: null, wipeProgress: 0,
            toggle() {
                this.active = !this.active;
                const btn = document.getElementById('genesis-toggle');
                Kernel.updateCell(90, this.active ? "genesis-active" : "active");
                if(this.active) { if(btn) btn.classList.add('active-op-genesis'); this.start(); } else { if(btn) btn.classList.remove('active-op-genesis'); cancelAnimationFrame(this.loop); }
            },
            start() {
                const c = document.getElementById('genesis-canvas'); if(!c) return;
                ...
                const draw = () => {
                    ...
                    ctx.strokeStyle = "var(--tabula-rasa-genesis)"; ctx.strokeRect(c.width/2-20, c.height/2-20, 40, 40);
                    this.loop = requestAnimationFrame(draw);
                }; draw();
            }
        };
        ...
        const StaticMonitorEngine = {
            ...
            animate() {
                this.canvases.forEach(c_monitor => {
                    ...
                    if (c_monitor.id === 'genesis-canvas' && TabulaRasaGenesisEngine.active) return;
        ...
        window.TabulaRasaGenesisEngine = TabulaRasaGenesisEngine;
