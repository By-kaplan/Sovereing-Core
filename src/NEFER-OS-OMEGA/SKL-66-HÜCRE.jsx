...
<head>
    ...
    <title>NEFER-OS | 1.0.0-OMEGA | 66. HÜCRE (HARDWARE OVERRIDE)</title>
    <style>
        :root {
            --bg-deep: #020617;
            --panel-bg: #0f172a;
            ...
            --hardware-override: #10b981; /* 66. Hücre Rengi: İtaat Zümrütü */
            --border: #1e293b;
            --cell-gap: 4px;
        }
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; cursor: crosshair; }
        ...
        .nefer-header {
            ...
            align-items: center;
            border-left: 4px solid var(--hardware-override);
        }

        .nefer-header h1 { font-size: 1.2rem; color: var(--hardware-override); letter-spacing: 3px; font-weight: 800; }
        ...
        .cell.override-active { background: var(--hardware-override); box-shadow: 0 0 15px var(--hardware-override); animation: obedience-pulse 1s infinite alternate; }
        .cell.loading { background: var(--warning); animation: pulse 1s infinite; }

        @keyframes obedience-pulse { 0% { opacity: 1; transform: scale(1); filter: brightness(1); } 100% { opacity: 0.6; transform: scale(1.1); filter: brightness(1.4); } }
        ...
        .viz-view {
            ...
            grid-template-rows: repeat(84, 35px) 1fr;
            gap: 2px;
            overflow-y: auto;
        }
        ...
        .log-entry.override { border-left-color: var(--hardware-override); color: var(--hardware-override); font-weight: bold; }
        ...
        .btn.active-op-override { border-color: var(--hardware-override); background: #064e3b; color: white; }
...
<body>
    ...
    <header class="nefer-header">
        <div>
            <h1>NEFER-OS // HARDWARE_OBEDIENCE_OVERRIDE</h1>
            <div style="font-size: 0.6rem; color: var(--text-dim);">66. HÜCRE: DONANIMSAL İTAAT GASPI VE I/O EGEMENLİĞİ</div>
        </div>
        <div class="meta-info">
    ...
    <div class="console-container">
        <div class="main-display">
            <div class="viz-view" id="viz-container">
                ...
                <!-- 66. Hücre: Hardware Override Vizüalizasyonu -->
                <div class="viz-box" style="grid-column: span 6; grid-row: span 4;">
                    <div style="position:absolute; top:5px; left:10px; font-size:0.4rem; color:var(--hardware-override); z-index:5;">I/O_BUS_PRIORITY_HIJACK_CONTROLLER</div>
                    <canvas id="override-canvas"></canvas>
                    <div id="override-hud" class="hud-text" style="color: var(--hardware-override);">FIRMWARE: OVERRIDDEN | I/O_GATE: LOCKED | OS_COMMANDS: FILTERED</div>
                </div>
            </div>
        ...
        <aside class="tools-panel">
            ...
            <div style="margin-top: auto; font-size: 0.55rem; color: var(--text-dim); border-top: 1px solid var(--border); padding-top: 10px;">
                HÜCRE 66: Doğrudan firmware seviyesinde yetki kurarak OS komutlarını baypas eder.
            </div>
        </aside>
    </div>
    ...
    <footer class="protocol-footer">
        <div class="attribution">
            <p><b>66. HÜCRE: HARDWARE OBEDIENCE OVERRIDE</b> - Donanımsal Gasp Protokolü</p>
    ...
    <script type="module">
        const Kernel = {
            init() {
                ...
                this.log("Kernel aktif. Donanımsal Gasp (Hardware Override) modülü mühürlendi.", "boot");
            },
            setupGrid() {
            ...
            async sequenceBoot() {
                ...
                this.log("66 Hücre donanımsal egemenlik senkronizasyonuna girdi.", "sys");
            }
        };
        ...
        const HardwareOverrideEngine = {
            ...
            toggle() {
                ...
                if(this.active) {
                    ...
                    Kernel.updateCell(66, "override-active");
                    Kernel.log("DONANIMSAL GASP AKTİF: I/O VERİYOLU ÜZERİNDE ÖNCELİK GASPI BAŞLADI.", "override");
                    this.start();
                } else {
                    if(btn) btn.classList.remove('active-op-override');
                    if(statusText) statusText.textContent = "KAPALI";
                    Kernel.updateCell(66, "active");
                    cancelAnimationFrame(this.loop);
                    this.interceptCount = 0;
                    if(hud) hud.textContent = "FIRMWARE: STANDBY | I/O_GATE: OPEN | STATE: IDLE";
                    Kernel.log("Donanımsal gasp protokolü durduruldu (Kontrol OS katmanına devredildi).", "sys");
                }
            },
            ...
            start() {
                ...
                this.busSignals = Array.from({length: 12}, (_, i) => ({
                    y: (c.height / 14) * (i + 1),
                    x: 0,
                ...
                const draw = () => {
                    ...
                    const hud = document.getElementById('override-hud');
                    if(hud) hud.textContent = `FIRMWARE: HIJACKED | INTERCEPTED: ${this.interceptCount} | OS_COMMANDS: FILTERING`;

                    // I/O Veriyolu Vizüalizasyonu
                    ctx.strokeStyle = "rgba(16, 185, 129, 0.2)";
                    ctx.lineWidth = 1;
                    this.busSignals.forEach(sig => {
                        ctx.beginPath();
                        ctx.moveTo(0, sig.y);
                        ctx.lineTo(c.width, sig.y);
                        ctx.stroke();
                        ...
                        const isOSCommand = Math.random() > 0.8;
                        ctx.fillStyle = isOSCommand ? "var(--hazard)" : "var(--hardware-override)";
                        ctx.fillRect(sig.x, sig.y - 4, 10, 8);
                        
                        if(isOSCommand && sig.x > c.width / 2) {
                            // Interception effect
                            ctx.fillStyle = "var(--hardware-override)";
                            ctx.fillText("NOP", sig.x, sig.y - 10);
                        }
                    });

                    if(Math.random() > 0.99) {
                        Kernel.log(`HardwareOverride: OS 'SHUTDOWN' komutu yakalandı ve 'NOP' ile değiştirildi.`, "override");
                    }
        ...
        const CognitiveHijackEngine = {
            active: false, loop: null, baitLevel: 0,
            toggle() {
                this.active = !this.active;
                const btn = document.getElementById('hijack-toggle');
                Kernel.updateCell(65, this.active ? "hijack-active" : "active");
                if(this.active) {
            ...
            start() {
                ...
                const draw = () => {
                    ...
                    for(let i=0; i<5; i++) ctx.fillRect(Math.random()*c.width, Math.random()*c.height, 4, 4);
                    this.loop = requestAnimationFrame(draw);
                }; draw();
            }
        };
