...
<head>
    ...
    <title>NEFER-OS | 1.0.0-OMEGA | 34. HÜCRE (SHADOW DOM)</title>
    <style>
        :root {
            --bg-deep: #020617;
            --panel-bg: #0f172a;
            ...
            --heap-spray: #f472b6;
            --time-bomb: #f59e0b;
            --shadow-dom: #6366f1; /* 34. Hücre Rengi: Gölge İndigo */
            --border: #1e293b;
            --cell-gap: 4px;
        }
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; cursor: crosshair; }
        ...
        .nefer-header {
            ...
            align-items: center;
            border-left: 4px solid var(--shadow-dom);
        }

        .nefer-header h1 { font-size: 1.2rem; color: var(--shadow-dom); letter-spacing: 3px; font-weight: 800; }
        ...
        .cell.shadow-active { background: var(--shadow-dom); box-shadow: 0 0 15px var(--shadow-dom); animation: shadow-flicker 0.6s infinite; }
        .cell.loading { background: var(--warning); animation: pulse 1s infinite; }

        @keyframes shadow-flicker { 0% { opacity: 1; } 50% { opacity: 0.4; } 100% { opacity: 1; } }
        ...
        .log-entry.shadow { border-left-color: var(--shadow-dom); color: var(--shadow-dom); font-weight: bold; }
        ...
        .btn.active-op-shadow { border-color: var(--shadow-dom); background: #312e81; color: white; }
...
<body>
    ...
    <header class="nefer-header">
        <div>
            <h1>NEFER-OS // SHADOW_DOM_EXFIL</h1>
            <div style="font-size: 0.6rem; color: var(--text-dim);">34. HÜCRE: GÖLGE-DOM VERİ KAÇIRMA VE İZOLASYON</div>
        </div>
        <div class="meta-info">
    ...
    <div class="console-container">
        <div class="main-display">
            <div class="viz-view">
                ...
                <div class="viz-box">
                    <div style="position:absolute; top:5px; left:10px; font-size:0.4rem; color:var(--shadow-dom); z-index:5;">SHADOW_DOM_ISOLATOR</div>
                    <canvas id="shadow-canvas"></canvas>
                    <div id="shadow-hud" class="hud-text" style="color: var(--shadow-dom);">SHADOW_ROOT: INVISIBLE | ACCESS: DENIED</div>
                </div>
                <div class="viz-box" style="grid-column: span 7;">
        ...
        <aside class="tools-panel">
            <button class="btn" onclick="BombEngine.toggle()">
                <span>33: TIME-SYNC LOGIC BOMB</span>
                <span>STATE</span>
            </button>
            <button class="btn" id="shadow-toggle" onclick="ShadowEngine.toggle()">
            ...
            <div style="margin-top: auto; font-size: 0.55rem; color: var(--text-dim); border-top: 1px solid var(--border); padding-top: 10px;">
                HÜCRE 34: Kritik verileri DOM ağacından saklayarak botların ve Bot Denetçilerinin erişimini engeller.
            </div>
        </aside>
    </div>
    ...
    <footer class="protocol-footer">
        <div class="attribution">
            <p><b>34. HÜCRE: SHADOW-DOM DATA EXFILTRATION</b> - Dijital Perde Protokolü</p>
    ...
    <script type="module">
        const Kernel = {
            ...
            async sequenceBoot() {
                for(let i=1; i<=34; i++) {
                    if([1, 24, 28, 29, 30, 31, 32, 33, 34].includes(i)) continue;
        ...
        const ShadowEngine = {
            ...
            toggle() {
                ...
                if(this.active) {
                    btn.classList.add('active-op-shadow');
                    document.getElementById('shadow-status').textContent = "GİZLENDİ";
                    Kernel.updateCell(34, "shadow-active");
                    Kernel.log("GÖLGE-DOM AKTİF: VERİLER KAPALI KAPSÜLLERE TAŞINDI.", "shadow");
                    this.startViz();
                } else {
                    btn.classList.remove('active-op-shadow');
                    document.getElementById('shadow-status').textContent = "KAPALI";
                    Kernel.updateCell(34, "active");
                    cancelAnimationFrame(this.loop);
                    hud.textContent = "SHADOW_ROOT: INVISIBLE | ACCESS: DENIED";
                    Kernel.log("Gölge perdeleri kaldırıldı.", "sys");
                }
            },
            ...
            startViz() {
                ...
                const draw = () => {
                    ...
                    this.scanY = (this.scanY + 1) % c.height;
                    
                    // Kapsül vizüalizasyonu
                    ...
                    // Rastgele "Karanlık" noktalar
                    for(let i=0; i<5; i++) {
                        ctx.fillStyle = "rgba(99, 102, 241, 0.3)";
                        ctx.fillRect(Math.random()*c.width, Math.random()*c.height, 4, 4);
                    }

                    document.getElementById('shadow-hud').textContent = `SHADOW_ROOT: ATTACHED | SCAN_LINE: ${Math.floor(this.scanY)}px`;

                    if(Math.random() > 0.98) {
                        const rootId = Math.random().toString(36).substring(7);
                        Kernel.log(`Shadow-DOM: Kritik veri root_{${rootId}} içine hapsedildi.`, "shadow");
                    }
