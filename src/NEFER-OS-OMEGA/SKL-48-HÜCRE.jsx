...
<head>
    ...
    <title>NEFER-OS | 1.0.0-OMEGA | 48. HÜCRE (NEURAL POISON)</title>
    <style>
        :root {
            --bg-deep: #020617;
            --panel-bg: #0f172a;
            ...
            --neural-poison: #d946ef; /* 48. Hücre Rengi: Zehirli Magenta */
            --border: #1e293b;
            --cell-gap: 4px;
        }
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; cursor: crosshair; }
        ...
        .nefer-header {
            ...
            align-items: center;
            border-left: 4px solid var(--neural-poison);
        }

        .nefer-header h1 { font-size: 1.2rem; color: var(--neural-poison); letter-spacing: 3px; font-weight: 800; }
        ...
        .cell.neural-poison-active { background: var(--neural-poison); box-shadow: 0 0 15px var(--neural-poison); animation: poison-pulse 1.5s infinite ease-in-out; }
        .cell.loading { background: var(--warning); animation: pulse 1s infinite; }

        @keyframes poison-pulse { 0% { opacity: 1; transform: scale(1); } 50% { opacity: 0.4; transform: scale(1.1); } 100% { opacity: 1; transform: scale(1); } }
        ...
        .viz-view {
            ...
            grid-template-rows: repeat(14, 40px) 1fr;
            gap: 2px;
            overflow-y: auto;
        }
        ...
        .log-entry.collision { border-left-color: var(--data-collision); color: var(--data-collision); }
        .log-entry.neural { border-left-color: var(--neural-poison); color: var(--neural-poison); font-weight: bold; }
...
<body>
    ...
    <header class="nefer-header">
        <div>
            <h1>NEFER-OS // NEURAL_NETWORK_WEIGHT_POISONING</h1>
            <div style="font-size: 0.6rem; color: var(--text-dim);">48. HÜCRE: YAPAY SİNİR AĞI AĞIRLIK ZEHİRLEMESİ VE KARAR SINIRI SABOTAJI</div>
        </div>
        <div class="meta-info">
    ...
    <div class="console-container">
        ...
        <aside class="tools-panel">
            <button class="btn" onclick="CollisionEngine.toggle()">
                <span>47: STRATEGIC DATA COLLISION</span>
                <span>STATE</span>
            </button>
            <button class="btn" id="neural-toggle" onclick="NeuralPoisonEngine.toggle()">
            ...
            <div style="margin-top: auto; font-size: 0.55rem; color: var(--text-dim); border-top: 1px solid var(--border); padding-top: 10px;">
                HÜCRE 48: AI modellerine bozuk mantık enjekte ederek karar sınırlarını saptırır.
            </div>
        </aside>
    </div>
    ...
    <footer class="protocol-footer">
        <div class="attribution">
            <p><b>48. HÜCRE: NEURAL NETWORK WEIGHT POISONING</b> - AI Karar Sabotajı</p>
    ...
    <script type="module">
        const Kernel = {
            init() {
                ...
                this.log("Kernel aktif. AI Zehirleme (Neural Poison) modülü hazır.", "boot");
            },
            setupGrid() {
            ...
            async sequenceBoot() {
                for(let i=1; i<=48; i++) {
                    if([1, 24, 47, 48].includes(i)) continue;
        ...
        const NeuralPoisonEngine = {
            ...
            toggle() {
                ...
                if(this.active) {
                    if(btn) btn.classList.add('active-op-neural');
                    if(statusText) statusText.textContent = "ZEHİRLİYOR";
                    Kernel.updateCell(48, "neural-poison-active");
                    Kernel.log("AI ZEHİRLEME AKTİF: KARAR SINIRLARI SAPTIRILIYOR.", "neural");
                    this.start();
                } else {
                    if(btn) btn.classList.remove('active-op-neural');
                    if(statusText) statusText.textContent = "KAPALI";
                    Kernel.updateCell(48, "active");
                    cancelAnimationFrame(this.loop);
                    this.bias = 0;
                    if(hud) hud.textContent = "MODEL: ENEMY_AI_V4 | BIAS: +0.00 | STATE: STANDBY";
                    Kernel.log("Yapay sinir ağı zehirleme protokolü durduruldu.", "sys");
                }
            },
            ...
            start() {
                ...
                this.points = Array.from({length: 40}, () => ({
                    ...
                    type: Math.random() > 0.5 ? 'threat' : 'normal'
                }));
                ...
                const draw = () => {
                    ...
                    this.bias += 0.001;
                    const hud = document.getElementById('neural-hud');
                    if(hud) hud.textContent = `MODEL: ENEMY_AI_V4 | BIAS: +${this.bias.toFixed(3)} | STATE: POISONING`;

                    ctx.strokeStyle = "var(--neural-poison)";
                    ctx.lineWidth = 2;
                    ctx.beginPath();
                    const boundaryOffset = Math.sin(Date.now() / 1000) * 10 + (this.bias * 50);
                    ctx.moveTo(0, c.height / 2 + boundaryOffset);
                    ctx.bezierCurveTo(c.width / 3, c.height / 2 - 20 + boundaryOffset, 2 * c.width / 3, c.height / 2 + 20 + boundaryOffset, c.width, c.height / 2 + boundaryOffset);
                    ctx.stroke();

                    this.points.forEach(p => {
                        const isPoisoned = p.type === 'threat' && p.y > (c.height / 2 + boundaryOffset);
                        ctx.fillStyle = isPoisoned ? "var(--safety)" : (p.type === 'threat' ? "var(--hazard)" : "var(--safety)");
                        ctx.beginPath();
                        ctx.arc(p.x, p.y, 2, 0, Math.PI * 2);
                        ctx.fill();
                        
                        if(isPoisoned && Math.random() > 0.95) {
                    ...
                    if(Math.random() > 0.98) {
                        Kernel.log(`Poison: Mikroskobik sapma enjekte edildi. Karar sınırı %${(this.bias * 10).toFixed(2)} kaydı.`, "neural");
                    }
        ...
        const CollisionEngine = {
            active: false, loop: null, buckets: 0,
            toggle() {
                this.active = !this.active;
                const btn = document.getElementById('collision-toggle');
                const hud = document.getElementById('collision-hud');
                const statusText = document.getElementById('collision-status');
                
                if(this.active) {
                    if(btn) btn.classList.add('active-op-collision');
                    if(statusText) statusText.textContent = "ÇARPIŞIYOR";
                    Kernel.updateCell(47, "collision-active");
                    Kernel.log("STRATEJİK ÇARPIŞMA AKTİF: HASH TABLOLARI ZEHİRLENİYOR.", "collision");
                    this.start();
                } else {
                    if(btn) btn.classList.remove('active-op-collision');
                    if(statusText) statusText.textContent = "KAPALI";
                    Kernel.updateCell(47, "active");
                    cancelAnimationFrame(this.loop);
                    this.buckets = 0;
                    if(hud) hud.textContent = "BUCKETS: 0 | COMPLEXITY: O(1) | STATUS: READY";
            ...
            start() {
                ...
                const draw = () => {
                    ...
                    this.buckets += 0.5;
                    for(let i=0; i<5; i++) ctx.fillRect(Math.random()*c.width, Math.random()*c.height, 15, 10);
                    this.loop = requestAnimationFrame(draw);
                }; draw();
            }
        };
        ...
        const HardwareTriggerEngine = { 
            active: false, loop: null, timings: Array(50).fill(100),
            toggle() {
                this.active = !this.active;
                const btn = document.getElementById('hardware-toggle');
                if(btn) this.active ? btn.classList.add('active-op-hardware') : btn.classList.remove('active-op-hardware');
        ...
        const BgpMirageEngine = { 
            ...
            toggle() {
                ...
                if(btn) this.active ? btn.classList.add('active-op-bgp') : btn.classList.remove('active-op-bgp');
