...
<head>
    ...
    <title>NEFER-OS | 1.0.0-OMEGA | 73. HÜCRE (HARDWARE TRUST)</title>
    <style>
        :root {
            --bg-deep: #020617;
            --panel-bg: #0f172a;
            ...
            --final-reset: #450a0a;
            --tabula-rasa: #ffffff;
            --hardware-trust: #94a3b8; /* 73. Hücre Rengi: Çelik Grisi / Güven Çekirdeği */
            --border: #1e293b;
            --cell-gap: 4px;
        }
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; cursor: crosshair; }
        ...
        .nefer-header {
            ...
            align-items: center;
            border-left: 4px solid var(--hardware-trust);
        }

        .nefer-header h1 { font-size: 1.2rem; color: var(--hardware-trust); letter-spacing: 3px; font-weight: 800; }
        ...
        .cell.trust-active { background: var(--hardware-trust); box-shadow: 0 0 15px var(--hardware-trust); animation: hardware-pulse 2s infinite alternate; }
        .cell.loading { background: var(--warning); animation: pulse 1s infinite; }

        @keyframes hardware-pulse { 0% { opacity: 1; filter: contrast(1); } 100% { opacity: 0.5; filter: contrast(1.5); transform: scale(1.1); } }
        ...
        .viz-view {
            ...
            grid-template-rows: repeat(112, 35px) 1fr;
            gap: 2px;
            overflow-y: auto;
        }
        ...
        .log-entry.trust { border-left-color: var(--hardware-trust); color: var(--hardware-trust); font-weight: bold; background: rgba(148, 163, 184, 0.1); }
        ...
        .btn.active-op-trust { border-color: var(--hardware-trust); background: #334155; color: white; border-width: 2px; }
...
<body>
    ...
    <header class="nefer-header">
        <div>
            <h1>NEFER-OS // HARDWARE_ROOT_OF_TRUST</h1>
            <div style="font-size: 0.6rem; color: var(--text-dim);">73. HÜCRE: DONANIMSAL GÜVEN ÇEKİRDEĞİ VE BARE-METAL İZOLASYONU</div>
        </div>
        <div class="meta-info">
    ...
    <div class="console-container">
        <div class="main-display">
            <div class="viz-view" id="viz-container">
                ...
                <!-- 73. Hücre: Hardware Trust Vizüalizasyonu -->
                <div class="viz-box" style="grid-column: span 6; grid-row: span 4;">
                    <div style="position:absolute; top:5px; left:10px; font-size:0.4rem; color:var(--hardware-trust); z-index:5;">HARDWARE_ROOT_OF_TRUST_PROTECTOR</div>
                    <canvas id="trust-canvas"></canvas>
                    <div id="trust-hud" class="hud-text" style="color: var(--hardware-trust);">FIRMWARE: VERIFIED | BUS: ISOLATED | RF_STATE: CUSTOM_ENCRYPTED</div>
                </div>
            </div>
        ...
        <aside class="tools-panel">
            ...
            <button class="btn" id="trust-toggle" onclick="HardwareTrustEngine.toggle()">
                ...
                <span id="trust-status">KAPALI</span>
            </button>
            ...
            <div style="margin-top: auto; font-size: 0.55rem; color: var(--text-dim); border-top: 1px solid var(--border); padding-top: 10px;">
                HÜCRE 73: Donanımı düşman tedarik zincirinden izole ederek bare-metal güvenliği sağlar.
            </div>
        </aside>
    </div>
    ...
    <footer class="protocol-footer">
        <div class="attribution">
            <p><b>73. HÜCRE: HARDWARE ROOT OF TRUST</b> - Donanımsal Güven Çekirdeği</p>
    ...
    <script type="module">
        const Kernel = {
            init() {
                ...
                this.log("Kernel aktif. Donanımsal Güven Çekirdeği (Hardware Trust) hazır.", "boot");
            },
            setupGrid() {
            ...
            async sequenceBoot() {
                const colors = ['active', 'boot-active', 'trust-active'];
                for(let i=1; i<=73; i++) {
                ...
                this.log("73 Hücre donanımsal kök-güven senkronizasyonuna hazır.", "sys");
            }
        };
        ...
        const HardwareTrustEngine = {
            ...
            toggle() {
                ...
                const hud = document.getElementById('trust-hud');

                if(this.active) {
                    if(btn) btn.classList.add('active-op-trust');
                    if(statusText) statusText.textContent = "BARE-METAL";
                    Kernel.updateCell(73, "trust-active");
                    Kernel.log("DONANIMSAL GÜVEN ÇEKİRDEĞİ AKTİF: COREBOOT KONTROLÜ ELE ALDI.", "trust");
                    this.start();
                } else {
                    if(btn) btn.classList.remove('active-op-trust');
                    if(statusText) statusText.textContent = "KAPALI";
                    Kernel.updateCell(73, "active");
                    cancelAnimationFrame(this.loop);
                    if(hud) hud.textContent = "FIRMWARE: VERIFIED | BUS: ISOLATED | RF_STATE: IDLE";
                    Kernel.log("Donanımsal güven çekirdeği askıya alındı.", "sys");
                }
            },

            start() {
                const c = document.getElementById('trust-canvas');
                if(!c) return;
                ...
                const draw = () => {
                    ...
                    const hud = document.getElementById('trust-hud');
                    if(hud) hud.textContent = `COREBOOT: ${this.integrity.toFixed(1)}% | BUS_ISOLATION: ACTIVE | RF_SECURE: 0x${Math.floor(Math.random()*1000).toString(16)}`;

                    // Donanım Devre Şeması Efekti
                    ctx.strokeStyle = "var(--hardware-trust)";
                    ctx.lineWidth = 1;
                    ...
                    for(let i=0; i<8; i++) {
                        ...
                        // Akış noktaları
                        const pulse = (Date.now() / 1000 + i) % 1;
                        ctx.fillStyle = "var(--hardware-trust)";
                        ctx.beginPath();
                        ctx.arc(centerX + Math.cos(angle)*(40 + pulse*60), centerY + Math.sin(angle)*(40 + pulse*60), 2, 0, Math.PI*2);
                        ctx.fill();
                    }

                    if(Math.random() > 0.99) {
                        Kernel.log(`HardwareTrust: WiFi/Bluetooth modülleri fiziksel olarak devre dışı bırakıldı.`, "trust");
                    }
