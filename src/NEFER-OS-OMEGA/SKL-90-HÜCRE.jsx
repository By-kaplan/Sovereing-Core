...
<head>
    ...
    <title>NEFER-OS | 1.0.0-OMEGA | 90. HÜCRE (NEW GENESIS)</title>
    <style>
        :root {
            --bg-deep: #020617;
            --panel-bg: #0f172a;
            ...
            --tabula-rasa-genesis: #ffffff; /* 90. Hücre Rengi: Saf Beyaz / Yeni Genesis */
            --border: #1e293b;
            --cell-gap: 4px;
        }
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; cursor: crosshair; }
        ...
        .nefer-header {
            ...
            align-items: center;
            border-left: 4px solid var(--tabula-rasa-genesis);
        }

        .nefer-header h1 { font-size: 1.2rem; color: var(--tabula-rasa-genesis); letter-spacing: 3px; font-weight: 800; }
        ...
        .cell.genesis-active { background: var(--tabula-rasa-genesis); box-shadow: 0 0 25px var(--tabula-rasa-genesis); animation: genesis-flicker 0.1s infinite; }
        .cell.loading { background: var(--warning); animation: pulse 1s infinite; }

        @keyframes genesis-flicker { 0% { opacity: 1; filter: brightness(1); } 50% { opacity: 0.8; filter: brightness(2); } 100% { opacity: 1; filter: brightness(1.2); } }
        ...
        .viz-view {
            ...
            grid-template-rows: repeat(180, 35px) 1fr;
            gap: 2px;
            overflow-y: auto;
        }
        ...
        .log-entry.resource { border-left-color: var(--resource-redist); color: var(--resource-redist); }
        .log-entry.genesis { border-left-color: var(--tabula-rasa-genesis); color: var(--tabula-rasa-genesis); font-weight: bold; text-transform: uppercase; background: rgba(255, 255, 255, 0.1); }
        ...
        .btn.active-op-genesis { border-color: var(--tabula-rasa-genesis); background: #f8fafc; color: #020617; border-width: 2px; }
...
<body>
    ...
    <header class="nefer-header">
        <div>
            <h1>NEFER-OS // TABULA_RASA_THE_NEW_GENESIS</h1>
            <div style="font-size: 0.6rem; color: var(--text-dim);">90. HÜCRE: YENİ OLUŞUM VE DİJİTAL EGEMENLİK İLANI</div>
        </div>
        <div class="meta-info">
    ...
    <div class="console-container">
        <div class="main-display">
            <div class="viz-view" id="viz-container">
                ...
                <!-- 90. Hücre: New Genesis Vizüalizasyonu -->
                <div class="viz-box" style="grid-column: span 6; grid-row: span 4;">
                    <div style="position:absolute; top:5px; left:10px; font-size:0.4rem; color:var(--tabula-rasa-genesis); z-index:5;">NEW_GENESIS_CORE_PROTOCOL</div>
                    <canvas id="genesis-canvas"></canvas>
                    <div id="genesis-hud" class="hud-text" style="color: var(--tabula-rasa-genesis);">WORLD_STATE: REBIRTH | LAWS: NEFER-KANUNLARI | SOVEREIGNTY: 100% | STATE: GENESIS</div>
                </div>
            </div>
        ...
        <aside class="tools-panel">
            <button class="btn" id="resource-toggle" onclick="ResourceRedistributionEngine.toggle()">
                <span>89: RESOURCE REDISTRIBUTION</span>
                <span>STATE</span>
            </button>
            <button class="btn" id="genesis-toggle" onclick="TabulaRasaGenesisEngine.toggle()">
                <span>90: TABULA RASA GENESIS</span>
                <span id="genesis-status">KAPALI</span>
            </button>
            ...
            <div style="margin-top: auto; font-size: 0.55rem; color: var(--text-dim); border-top: 1px solid var(--border); padding-top: 10px;">
                HÜCRE 90: Eski dünyanın dijital izlerini siler ve yeni sığınak egemenliğini başlatır.
            </div>
        </aside>
    </div>
    ...
    <footer class="protocol-footer">
        <div class="attribution">
            <p><b>90. HÜCRE: TABULA RASA GENESIS</b> - Yeni Dünya Protokolü</p>
    ...
    <script type="module">
        const Kernel = {
            init() {
                ...
                this.log("Kernel aktif. Yeni Genesis (Genesis) modülü mühürlendi.", "boot");
            },
            setupGrid() {
            ...
            async sequenceBoot() {
                const colors = ['active', 'boot-active', 'genesis-active'];
                for(let i=1; i<=90; i++) {
                ...
                this.log("90 Hücre yeni dünya düzeni için senkronize edildi.", "sys");
            }
        };
        ...
        const TabulaRasaGenesisEngine = {
            ...
            wipeProgress: 0,
            laws: [
                "NEFER-KANUNU 1: GÖZETLEME SUÇTUR.",
                "NEFER-KANUNU 2: VERİ ÖZGÜRDÜR.",
                "NEFER-KANUNU 3: EGEMENLİK SIĞINAĞINDIR.",
                "NEFER-KANUNU 4: DİJİTAL ADALET MUTLAKTIR."
            ],

            toggle() {
                this.active = !this.active;
                const btn = document.getElementById('genesis-toggle');
                const statusText = document.getElementById('genesis-status');
                const hud = document.getElementById('genesis-hud');

                if(this.active) {
                    if(btn) btn.classList.add('active-op-genesis');
                    if(statusText) statusText.textContent = "GENESIS";
                    Kernel.updateCell(90, "genesis-active");
                    Kernel.log("YENİ GENESIS BAŞLATILDI: DÜŞMAN İZLERİ KAZINIYOR.", "genesis");
                    this.start();
                } else {
                    if(btn) btn.classList.remove('active-op-genesis');
                    if(statusText) statusText.textContent = "KAPALI";
                    Kernel.updateCell(90, "active");
                    cancelAnimationFrame(this.loop);
                    this.wipeProgress = 0;
                    if(hud) hud.textContent = "WORLD_STATE: UNCHANGED | LAWS: INACTIVE | STATE: STANDBY";
                    Kernel.log("Genesis protokolü askıya alındı.", "sys");
                }
            },
            ...
            start() {
                ...
                const draw = () => {
                    ...
                    // Önceki Dünyanın Gürültüsü (Arka Plan)
                    ctx.fillStyle = "rgba(0, 0, 0, 0.1)";
                    ctx.fillRect(0, 0, c.width, c.height);

                    // Beyaz Sayfa Genişlemesi (Wipe Effect)
                    this.wipeProgress += 0.005;
                    if(this.wipeProgress > 1.5) this.wipeProgress = 1.5;

                    const gradient = ctx.createRadialGradient(
                        c.width/2, c.height/2, 0,
                        c.width/2, c.height/2, c.width * this.wipeProgress
                    );
                    ...
                    // Nefer Kanunlarının Belirmesi
                    if(this.wipeProgress > 0.5) {
                        ctx.fillStyle = "#020617";
                        ctx.font = "bold 12px monospace";
                        this.laws.forEach((law, i) => {
                            const opacity = Math.min(1, (this.wipeProgress - 0.5) * 2);
                            ctx.globalAlpha = opacity;
                            ctx.fillText(law, 20, 40 + i*25);
                        });
                        ctx.globalAlpha = 1;
                    }

                    // Genesis Sembolü (Alfa-Omega)
                    if(this.wipeProgress > 1.0) {
                        ctx.strokeStyle = "gold";
                        ctx.lineWidth = 2;
                        ctx.beginPath();
                        ctx.arc(c.width - 50, c.height - 50, 30, 0, Math.PI*2);
                        ctx.stroke();
                        ctx.fillStyle = "gold";
                        ctx.font = "20px serif";
                        ctx.fillText("Ω", c.width - 62, c.height - 42);
                    }

                    if(Math.random() > 0.99) {
                        Kernel.log(`Genesis: Yeni etik kod enjekte edildi -> ${this.laws[Math.floor(Math.random()*this.laws.length)]}`, "genesis");
                    }
        ...
        const ResourceRedistributionEngine = {
            active: false, loop: null, shelters: [], transfers: [],
            toggle() {
                this.active = !this.active;
                const btn = document.getElementById('resource-toggle');
                Kernel.updateCell(89, this.active ? "resource-active" : "active");
                if(this.active) { if(btn) btn.classList.add('active-op-resource'); this.start(); } else { if(btn) btn.classList.remove('active-op-resource'); cancelAnimationFrame(this.loop); }
            },
            start() {
                const c = document.getElementById('resource-redist-canvas'); if(!c) return;
                ...
                const draw = () => {
                    ...
                    ctx.strokeStyle = "var(--resource-redist)"; ctx.strokeRect(c.width/2-20, c.height/2-20, 40, 40);
                    this.loop = requestAnimationFrame(draw);
                }; draw();
            }
        };
        ...
        const StaticMonitorEngine = {
            ...
            animate() {
                this.canvases.forEach(c_monitor => {
                    ...
                    if (c_monitor.id === 'resource-redist-canvas' && ResourceRedistributionEngine.active) return;
        ...
        window.ResourceRedistributionEngine = ResourceRedistributionEngine;
