...
<head>
    ...
    <title>NEFER-OS | 1.0.0-OMEGA | 82. HÜCRE (AI FOG)</title>
    <style>
        :root {
            --bg-deep: #020617;
            --panel-bg: #0f172a;
            ...
            --sovereign-os: #f8fafc;
            --ai-fog: #a78bfa; /* 82. Hücre Rengi: Sis Moru */
            --border: #1e293b;
            --cell-gap: 4px;
        }
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; cursor: crosshair; }
        ...
        .nefer-header {
            ...
            align-items: center;
            border-left: 4px solid var(--ai-fog);
        }

        .nefer-header h1 { font-size: 1.2rem; color: var(--ai-fog); letter-spacing: 3px; font-weight: 800; }
        ...
        .cell.fog-active { background: var(--ai-fog); box-shadow: 0 0 15px var(--ai-fog); animation: fog-swirl 3s infinite ease-in-out; }
        .cell.loading { background: var(--warning); animation: pulse 1s infinite; }

        @keyframes fog-swirl { 0%, 100% { transform: scale(1); opacity: 1; } 50% { transform: scale(1.3) rotate(90deg); opacity: 0.6; } }
        ...
        .viz-view {
            ...
            grid-template-rows: repeat(148, 35px) 1fr;
            gap: 2px;
            overflow-y: auto;
        }
        ...
        .log-entry.fog { border-left-color: var(--ai-fog); color: var(--ai-fog); font-weight: bold; background: rgba(167, 139, 250, 0.1); }
        ...
        .btn.active-op-fog { border-color: var(--ai-fog); background: #4c1d95; color: white; border-width: 2px; }
...
<body>
    ...
    <header class="nefer-header">
        <div>
            <h1>NEFER-OS // AI_COUNTER_INTELLIGENCE_FOG</h1>
            <div style="font-size: 0.6rem; color: var(--text-dim);">82. HÜCRE: YAPAY ZEKA KARŞI-İSTİHBARAT SİSİ VE DİJİTAL KOKU</div>
        </div>
        <div class="meta-info">
    ...
    <div class="console-container">
        <div class="main-display">
            <div class="viz-view" id="viz-container">
                ...
                <!-- 82. Hücre: AI Fog Vizüalizasyonu -->
                <div class="viz-box" style="grid-column: span 6; grid-row: span 4;">
                    <div style="position:absolute; top:5px; left:10px; font-size:0.4rem; color:var(--ai-fog); z-index:5;">AI_DECEPTION_SCENT_GENERATOR</div>
                    <canvas id="fog-canvas"></canvas>
                    <div id="fog-hud" class="hud-text" style="color: var(--ai-fog);">SCENT: SMART_FRIDGE_EMU | NOISE: HIGH | GHOST_NODES: 14 | STATE: CAMOUFLAGED</div>
                </div>
            </div>
        ...
        <aside class="tools-panel">
            ...
            <button class="btn" id="fog-toggle" onclick="AICounterIntelligenceFogEngine.toggle()">
                ...
                <span id="fog-status">KAPALI</span>
            </button>
            ...
            <div style="margin-top: auto; font-size: 0.55rem; color: var(--text-dim); border-top: 1px solid var(--border); padding-top: 10px;">
                HÜCRE 82: Düşman AI'larını yanıltıcı telemetri ve dijital kokularla manipüle eder.
            </div>
        </aside>
    </div>
    ...
    <footer class="protocol-footer">
        <div class="attribution">
            <p><b>82. HÜCRE: AI COUNTER-INTELLIGENCE FOG</b> - Karşı-İstihbarat Protokolü</p>
    ...
    <script type="module">
        const Kernel = {
            init() {
                ...
                this.log("Kernel aktif. AI Karşı-İstihbarat Sisi (AI Fog) modülü mühürlendi.", "boot");
            },
            setupGrid() {
            ...
            async sequenceBoot() {
                const colors = ['active', 'boot-active', 'fog-active'];
                for(let i=1; i<=82; i++) {
                ...
                this.log("82 Hücre tam kamuflaj ve yanıltma için senkronize edildi.", "sys");
            }
        };
        ...
        const AICounterIntelligenceFogEngine = {
            ...
            loop: null,
            particles: [],
            scents: ["SMART_FRIDGE_EMU", "OLD_TRANSFORMER_NOISE", "BASE_STATION_ERR", "TRAFFIC_LIGHT_HUB", "GHOST_SHELTER_V2"],
            currentScent: 0,

            toggle() {
                this.active = !this.active;
                const btn = document.getElementById('fog-toggle');
                const statusText = document.getElementById('fog-status');
                const hud = document.getElementById('fog-hud');

                if(this.active) {
                    if(btn) btn.classList.add('active-op-fog');
                    if(statusText) statusText.textContent = "SİSLENDİ";
                    Kernel.updateCell(82, "fog-active");
                    Kernel.log("AI FOG AKTİF: YANILTICI DİJİTAL KOKU YAYILIYOR.", "fog");
                    this.start();
                } else {
                    if(btn) btn.classList.remove('active-op-fog');
                    if(statusText) statusText.textContent = "KAPALI";
                    Kernel.updateCell(82, "active");
                    cancelAnimationFrame(this.loop);
                    if(hud) hud.textContent = "SCENT: NONE | NOISE: LOW | STATE: EXPOSED";
                    Kernel.log("AI Karşı-İstihbarat protokolü askıya alındı.", "sys");
                }
            },
            ...
            start() {
                ...
                const draw = () => {
                    ...
                    const hud = document.getElementById('fog-hud');
                    if(hud) hud.textContent = `SCENT: ${this.scents[this.currentScent]} | NOISE: 0x${Math.floor(Math.random()*0xFFFF).toString(16)} | STATE: MASKED`;

                    // Sis Parçacıkları (Yanıltıcı Koku)
                    this.particles.forEach(p => {
                        ...
                        // "Hata" efektleri (SubtleCrypto simülasyonu)
                        if(Math.random() > 0.98) {
                            ctx.fillStyle = "white";
                            ctx.font = "6px monospace";
                            ctx.fillText(Math.random().toString(36).substring(7), p.x, p.y);
                        }
                    });

                    // Hayalet Hedef Koordinatları (Radar)
                    ctx.strokeStyle = "rgba(167, 139, 250, 0.2)";
                    ctx.beginPath();
                    ctx.arc(c.width/2, c.height/2, 50 + Math.sin(Date.now()/500)*10, 0, Math.PI*2);
                    ctx.stroke();

                    if(Math.random() > 0.995) {
                        this.currentScent = (this.currentScent + 1) % this.scents.length;
                        Kernel.log(`AI Fog: Scent güncellendi -> ${this.scents[this.currentScent]}`, "fog");
                    }
        ...
        const SovereignOSEngine = {
            active: false, loop: null, memoryMap: [], cycles: 0,
            toggle() {
                this.active = !this.active;
                const btn = document.getElementById('sovereign-toggle');
                Kernel.updateCell(81, this.active ? "sovereign-active" : "active");
                if(this.active) { if(btn) btn.classList.add('active-op-sovereign'); this.start(); } else { if(btn) btn.classList.remove('active-op-sovereign'); cancelAnimationFrame(this.loop); }
            },
            start() {
                const c = document.getElementById('sovereign-canvas'); if(!c) return;
                ...
                const draw = () => {
                    ...
                    ctx.strokeStyle = "rgba(248, 250, 252, 0.1)"; ctx.strokeRect(c.width/2-20, c.height/2-20, 40, 40);
                    this.loop = requestAnimationFrame(draw);
                }; draw();
            }
        };
        ...
        const StaticMonitorEngine = {
            ...
            animate() {
                this.canvases.forEach(c => {
                    ...
                    if (c.id === 'fog-canvas' && AICounterIntelligenceFogEngine.active) return;
