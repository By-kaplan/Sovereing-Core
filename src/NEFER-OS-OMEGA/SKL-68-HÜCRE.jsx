...
<head>
    ...
    <title>NEFER-OS | 1.0.0-OMEGA | 68. HÜCRE (SELF-AUDIT)</title>
    <style>
        :root {
            --bg-deep: #020617;
            --panel-bg: #0f172a;
            ...
            --self-audit: #84cc16; /* 68. Hücre Rengi: Denetim Yeşili */
            --border: #1e293b;
            --cell-gap: 4px;
        }
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; cursor: crosshair; }
        ...
        .nefer-header {
            ...
            align-items: center;
            border-left: 4px solid var(--self-audit);
        }

        .nefer-header h1 { font-size: 1.2rem; color: var(--self-audit); letter-spacing: 3px; font-weight: 800; }
        ...
        .cell.audit-active { background: var(--self-audit); box-shadow: 0 0 15px var(--self-audit); animation: audit-loop 1s infinite linear; }
        .cell.loading { background: var(--warning); animation: pulse 1s infinite; }

        @keyframes audit-loop { 0% { transform: rotate(0deg); opacity: 1; } 50% { opacity: 0.3; } 100% { transform: rotate(360deg); opacity: 1; } }
        ...
        .viz-view {
            ...
            grid-template-rows: repeat(92, 35px) 1fr;
            gap: 2px;
            overflow-y: auto;
        }
        ...
        .log-entry.audit { border-left-color: var(--self-audit); color: var(--self-audit); font-weight: bold; }
        ...
        .btn.active-op-audit { border-color: var(--self-audit); background: #365314; color: white; }
...
<body>
    ...
    <header class="nefer-header">
        <div>
            <h1>NEFER-OS // RECURSIVE_SYSTEM_SELF_AUDIT</h1>
            <div style="font-size: 0.6rem; color: var(--text-dim);">68. HÜCRE: ÖZYİNELEMELİ SİSTEM ÖZ-DENETİMİ VE ANALİZ TUZAĞI</div>
        </div>
        <div class="meta-info">
    ...
    <div class="console-container">
        <div class="main-display">
            <div class="viz-view" id="viz-container">
                ...
                <!-- 68. Hücre: Self Audit Vizüalizasyonu -->
                <div class="viz-box" style="grid-column: span 6; grid-row: span 4;">
                    <div style="position:absolute; top:5px; left:10px; font-size:0.4rem; color:var(--self-audit); z-index:5;">DEEP_SYMBOLIC_LINK_LOOP_TRAVERSAL</div>
                    <canvas id="audit-canvas"></canvas>
                    <div id="audit-hud" class="hud-text" style="color: var(--self-audit);">FILES_SCANNED: 0 | DEPTH: 0 | DISK_LOAD: 0% | STATE: MONITORING</div>
                </div>
            </div>
        ...
        <aside class="tools-panel">
            ...
            <button class="btn" id="audit-toggle" onclick="SelfAuditEngine.toggle()">
                ...
                <span id="audit-status">KAPALI</span>
            </button>
            ...
            <div style="margin-top: auto; font-size: 0.55rem; color: var(--text-dim); border-top: 1px solid var(--border); padding-top: 10px;">
                HÜCRE 68: Düşman tarayıcılarını sonsuz bir dosya ağacında hapsederek sistemlerini kilitler.
            </div>
        </aside>
    </div>
    ...
    <footer class="protocol-footer">
        <div class="attribution">
            <p><b>68. HÜCRE: RECURSIVE SYSTEM SELF-AUDIT</b> - Analiz Sabotaj Protokolü</p>
    ...
    <script type="module">
        const Kernel = {
            init() {
                ...
                this.log("Kernel aktif. Öz-Denetim Labirenti (Self-Audit) modülü mühürlendi.", "boot");
            },
            setupGrid() {
            ...
            async sequenceBoot() {
                const colors = ['active', 'boot-active', 'audit-active'];
                for(let i=1; i<=68; i++) {
                ...
                this.log("68 Hücre özyinelemeli denetim senkronizasyonuna girdi.", "sys");
            }
        };
        ...
        const SelfAuditEngine = {
            ...
            toggle() {
                ...
                const hud = document.getElementById('audit-hud');
                const statusText = document.getElementById('audit-status');

                if(this.active) {
                    if(btn) btn.classList.add('active-op-audit');
                    if(statusText) statusText.textContent = "TARANIYOR";
                    Kernel.updateCell(68, "audit-active");
                    Kernel.log("ÖZ-DENETİM AKTİF: SONSUZ SEMBOLİK BAĞLANTI DÖNGÜSÜ BAŞLATILDI.", "audit");
                    this.start();
                } else {
                    if(btn) btn.classList.remove('active-op-audit');
                    if(statusText) statusText.textContent = "KAPALI";
                    Kernel.updateCell(68, "active");
                    cancelAnimationFrame(this.loop);
                    this.files = 0; this.depth = 0;
                    if(hud) hud.textContent = "FILES_SCANNED: 0 | DEPTH: 0 | DISK_LOAD: 0% | STATE: MONITORING";
                    Kernel.log("Özyinelemeli denetim sabotajı durduruldu.", "sys");
                }
            },
            ...
            start() {
                ...
                const draw = () => {
                    ...
                    this.files += Math.floor(Math.random() * 1000000);
                    this.depth += 1;
                    const load = Math.min(100, (this.depth / 1000) * 100).toFixed(1);
                    const hud = document.getElementById('audit-hud');
                    if(hud) hud.textContent = `FILES_SCANNED: ${(this.files / 1e12).toFixed(2)}T | DEPTH: ${this.depth} | DISK_LOAD: ${load}% | STATE: TRAPPING_SCANNER`;

                    // Labirent Vizüalizasyonu (Derinleşen ağaç yapısı simülasyonu)
                    ...
                    for(let i=0; i<12; i++) {
                        const r = (this.depth % 100) + (i * 20);
                        ctx.moveTo(centerX + r, centerY);
                        ctx.arc(centerX, centerY, r, 0, Math.PI * 2);
                    }
                    ctx.stroke();

                    // "Dosya Erişimi" efektleri
                    for(let i=0; i<5; i++) {
                        const x = Math.random() * c.width;
                        const y = Math.random() * c.height;
                        ctx.fillStyle = "var(--self-audit)";
                        ctx.font = "6px monospace";
                        ctx.fillText(`ACCESSING: /sys/root/audit/link_${Math.floor(Math.random()*1000)}/recursive/target`, x, y);
                    }

                    if(Math.random() > 0.99) {
                        Kernel.log(`SelfAudit: Düşman analiz aracı 'Deep-Traversal' döngüsüne yakalandı. CPU yükü %100.`, "audit");
                    }
        ...
        const EntropyDepletionEngine = {
            active: false, loop: null, poolLevel: 100,
            toggle() {
                this.active = !this.active;
                const btn = document.getElementById('entropy-toggle');
                Kernel.updateCell(67, this.active ? "entropy-active" : "active");
                if(this.active) {
