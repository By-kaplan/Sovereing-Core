...
<head>
    ...
    <title>NEFER-OS | 1.0.0-OMEGA | 25. HÜCRE (COGNITIVE DECOY)</title>
    <style>
        :root {
            --bg-deep: #020617;
            --panel-bg: #0f172a;
            ...
            --warning: #f59e0b;
            --bootloader: #fdf2f8;
            --decoy: #a78bfa; /* 25. Hücre Rengi: Olasılık Moru */
            --border: #1e293b;
            --cell-gap: 4px;
        }
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; cursor: crosshair; }
        ...
        .nefer-header {
            ...
            align-items: center;
            border-left: 4px solid var(--decoy);
        }

        .nefer-header h1 { font-size: 1.2rem; color: var(--decoy); letter-spacing: 3px; font-weight: 800; }
        ...
        .cell.decoy-active { background: var(--decoy); box-shadow: 0 0 15px var(--decoy); animation: probability-pulse 2s infinite; }
        .cell.loading { background: var(--warning); animation: pulse 1s infinite; }

        @keyframes probability-pulse { 0% { opacity: 1; } 50% { opacity: 0.4; } 100% { opacity: 1; } }
        ...
        .log-entry.boot { border-left-color: var(--bootloader); color: var(--bootloader); }
        .log-entry.decoy { border-left-color: var(--decoy); color: var(--decoy); font-weight: bold; }
...
<body>
    ...
    <header class="nefer-header">
        <div>
            <h1>NEFER-OS // COGNITIVE_DECOY</h1>
            <div style="font-size: 0.6rem; color: var(--text-dim);">25. HÜCRE: BİLİŞSEL YEM DİZİLERİ VE OLASILIK TÜNELLERİ</div>
        </div>
        <div class="meta-info">
    ...
    <div class="console-container">
        <div class="main-display">
            <div class="viz-view">
                <div class="viz-box">
                    ...
                    <div class="hud-text" style="color: var(--bootloader);">KERNEL_STATE: SEALED</div>
                </div>
                <div class="viz-box">
                    ...
                    <div id="decoy-hud" class="hud-text">MAZE_DEPTH: 0 | CPU_SINK: IDLE</div>
                </div>
            </div>
        ...
        <aside class="tools-panel">
            <button class="btn" id="decoy-toggle" onclick="DecoyEngine.toggle()">
            ...
            <div style="margin-top: auto; font-size: 0.55rem; color: var(--text-dim); border-top: 1px solid var(--border); padding-top: 10px;">
                HÜCRE 25: Yapay zeka motorlarını sonsuz bir rastgelelik tüneline sokarak kaynak tüketir.
            </div>
        </aside>
    </div>
    ...
    <footer class="protocol-footer">
        <div class="attribution">
            <p><b>25. HÜCRE: COGNITIVE DECOY ARRAYS</b> - AI/Quantum Sabotajı</p>
    ...
    <script type="module">
        ...
        const Kernel = {
            ...
            async sequenceBoot() {
                for(let i=1; i<=25; i++) {
                    if([1, 24, 25].includes(i)) continue;
        ...
        const DecoyEngine = {
            ...
            async toggle() {
                ...
                const hud = document.getElementById('decoy-hud');
                
                if(this.active) {
                    btn.classList.add('active-op-decoy');
                    document.getElementById('decoy-status').textContent = "AKTİF";
                    Kernel.updateCell(25, "decoy-active");
                    Kernel.log("BİLİŞSEL YEM DİZİLERİ AKTİF: OLASILIK TÜNELLERİ OLUŞTURULUYOR.", "decoy");
                    this.startViz();
                    this.startDBSabotage();
                } else {
                    ...
                    hud.textContent = "MAZE_DEPTH: 0 | CPU_SINK: IDLE";
                    Kernel.log("Yem dizileri pasifize edildi.", "sys");
                }
            },
            ...
            startViz() {
                ...
                const draw = () => {
                    ...
                    if(Math.random() > 0.95) {
                        Kernel.log(`Olasılık Hücresi Oluşturuldu: 0x${Math.random().toString(16).slice(2,8)}`, "decoy");
                        Kernel.log(`Hash Rotasyonu: Düşman AI motoru %${Math.floor(Math.random()*20)+80} yük altında.`, "decoy");
                    }
            ...
            startDBSabotage() {
                const request = indexedDB.open("NeferDecoyDB", 1);
                request.onupgradeneeded = (e) => {
                    const db = e.target.result;
                    db.createObjectStore("probability_cells", { keyPath: "id" });
                };
                request.onsuccess = (e) => {
            ...
            runDBCycle() {
                ...
                const tx = this.db.transaction("probability_cells", "readwrite");
                const store = tx.objectStore("probability_cells");
                for(let i=0; i<100; i++) {
                    store.put({ id: Math.random(), data: crypto.getRandomValues(new Uint8Array(256)) });
                }
                tx.oncomplete = () => setTimeout(() => this.runDBCycle(), 10);
            }
        };
