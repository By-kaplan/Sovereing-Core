...
<head>
    ...
    <title>NEFER-OS | 1.0.0-OMEGA | 87. HÜCRE (PULSE COLLAPSE)</title>
    <style>
        :root {
            --bg-deep: #020617;
            --panel-bg: #0f172a;
            ...
            --pulse-collapse: #ef4444; /* 87. Hücre Rengi: Yıkım Kırmızısı */
            --border: #1e293b;
            --cell-gap: 4px;
        }
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; cursor: crosshair; }
        ...
        .nefer-header {
            ...
            align-items: center;
            border-left: 4px solid var(--pulse-collapse);
        }

        .nefer-header h1 { font-size: 1.2rem; color: var(--pulse-collapse); letter-spacing: 3px; font-weight: 800; }
        ...
        .viz-view {
            ...
            grid-template-rows: repeat(168, 35px) 1fr;
            gap: 2px;
            overflow-y: auto;
        }
        ...
        .log-entry.pulse-collapse { border-left-color: var(--pulse-collapse); color: var(--pulse-collapse); font-weight: bold; background: rgba(239, 68, 68, 0.1); }
        ...
        .btn.active-op-pulse { border-color: var(--pulse-collapse); background: #450a0a; color: white; border-width: 2px; }
...
<body>
    ...
    <header class="nefer-header">
        <div>
            <h1>NEFER-OS // SYNCHRONIZED_PULSE_COLLAPSE</h1>
            <div style="font-size: 0.6rem; color: var(--text-dim);">87. HÜCRE: SENKRONİZE PALS ÇÖKÜŞÜ VE DONANIM YIKIMI</div>
        </div>
        <div class="meta-info">
    ...
    <div class="console-container">
        <div class="main-display">
            <div class="viz-view" id="viz-container">
                ...
                <!-- 87. Hücre: Synchronized Pulse Collapse Vizüalizasyonu -->
                <div class="viz-box" style="grid-column: span 6; grid-row: span 4;">
                    <div style="position:absolute; top:5px; left:10px; font-size:0.4rem; color:var(--pulse-collapse); z-index:5;">EM_NOISE_STORM_GENERATOR // DESTRUCTION_MODE</div>
                    <canvas id="pulse-collapse-canvas"></canvas>
                    <div id="pulse-collapse-hud" class="hud-text" style="color: var(--pulse-collapse);">NODES: 834 | COLLECTIVE_STRIKE: ENABLED | BUFFER_LOAD: 99.9% | STATE: COLLAPSING</div>
                </div>
            </div>
        ...
        <aside class="tools-panel">
            ...
            <button class="btn" id="pulse-collapse-toggle" onclick="SynchronizedPulseCollapseEngine.toggle()">
                ...
                <span id="pulse-collapse-status">KAPALI</span>
            </button>
            ...
            <div style="margin-top: auto; font-size: 0.55rem; color: var(--text-dim); border-top: 1px solid var(--border); padding-top: 10px;">
                HÜCRE 87: Küresel sığınak ağı ile düşman donanımını fiziksel olarak felç eder.
            </div>
        </aside>
    </div>
    ...
    <footer class="protocol-footer">
        <div class="attribution">
            <p><b>87. HÜCRE: SYNCHRONIZED PULSE COLLAPSE</b> - Yıkım Modu Protokolü</p>
    ...
    <script type="module">
        const Kernel = {
            init() {
                ...
                this.log("Kernel aktif. Senkronize Pals Çöküşü (Pulse Collapse) modülü mühürlendi.", "boot");
            },
            setupGrid() {
            ...
            async sequenceBoot() {
                const colors = ['active', 'boot-active', 'pulse-collapse-active'];
                for(let i=1; i<=87; i++) {
                ...
                this.log("87 Hücre küresel yıkım modu senkronizasyonuna girdi.", "sys");
            }
        };
        ...
        const SynchronizedPulseCollapseEngine = {
            ...
            toggle() {
                ...
                const statusText = document.getElementById('pulse-collapse-status');
                const hud = document.getElementById('pulse-collapse-hud');

                if(this.active) {
                    if(btn) btn.classList.add('active-op-pulse');
                    if(statusText) statusText.textContent = "YIKILIYOR";
                    Kernel.updateCell(87, "pulse-collapse-active");
                    Kernel.log("YIKIM MODU AKTİF: KÜRESEL PALS ÇÖKÜŞÜ TETİKLENDİ.", "pulse-collapse");
                    this.start();
                } else {
                    if(btn) btn.classList.remove('active-op-pulse');
                    if(statusText) statusText.textContent = "KAPALI";
                    Kernel.updateCell(87, "active");
                    cancelAnimationFrame(this.loop);
                    if(hud) hud.textContent = "NODES: 0 | COLLECTIVE: OFF | STATE: STANDBY";
                    Kernel.log("Yıkım protokolü askıya alındı.", "sys");
                }
            },

            start() {
                const c = document.getElementById('pulse-collapse-canvas');
                if(!c) return;
                ...
                const draw = () => {
                    ...
                    const hud = document.getElementById('pulse-collapse-hud');
                    if(hud) hud.textContent = `NODES: 834 | BURST_POWER: ${(Math.random()*100).toFixed(2)}GW | LOAD: 100% | STATE: COLLAPSING`;

                    // Şok Dalgaları (Shockwaves)
                    if(Math.random() > 0.95) {
                        this.shockwaves.push({ x: c.width/2, y: c.height/2, r: 0, alpha: 1 });
                    }

                    this.shockwaves.forEach((w, i) => {
                        w.r += 5;
                        w.alpha -= 0.01;
                        if(w.alpha <= 0) this.shockwaves.splice(i, 1);
                    ...
                    // Isınan/Yanan Donanım Parçacıkları (Debris)
                    if(Math.random() > 0.7) {
                        this.debris.push({ 
                    ...
                    this.debris.forEach((d, i) => {
                        ...
                        ctx.fillStyle = Math.random() > 0.5 ? "var(--pulse-collapse)" : "white";
                    ...
                    if(Math.random() > 0.99) {
                        Kernel.log(`ALERT: Düşman Router_ID_${Math.floor(Math.random()*9999)} buffer overflow nedeniyle yandı!`, "hazard");
                    }
        ...
        const LogicGateOverrideEngine = {
            active: false, loop: null, gates: [], signals: [],
            toggle() {
                this.active = !this.active;
                const btn = document.getElementById('override-toggle');
                Kernel.updateCell(86, this.active ? "override-active" : "active");
                if(this.active) { if(btn) btn.classList.add('active-op-override'); this.start(); } else { if(btn) btn.classList.remove('active-op-override'); cancelAnimationFrame(this.loop); }
            },
            start() {
                const c = document.getElementById('override-canvas'); if(!c) return;
                ...
                const draw = () => {
                    ...
                    ctx.strokeStyle = "var(--logic-gate-override)"; ctx.strokeRect(c.width/2-20, c.height/2-20, 40, 40);
                    this.loop = requestAnimationFrame(draw);
                }; draw();
            }
        };
