...
<head>
    ...
    <title>NEFER-OS | 1.0.0-OMEGA | 41. HÜCRE (HEURISTIC SABOTAGE)</title>
    <style>
        :root {
            --bg-deep: #020617;
            --panel-bg: #0f172a;
            ...
            --stegano: #4ade80;
            --mutation-key: #ec4899;
            --heuristic-sabotage: #818cf8; /* 41. Hücre Rengi: Sezgisel Mor-Mavi */
            --border: #1e293b;
            --cell-gap: 4px;
        }
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; cursor: crosshair; }
        ...
        .nefer-header {
            ...
            align-items: center;
            border-left: 4px solid var(--heuristic-sabotage);
        }

        .nefer-header h1 { font-size: 1.2rem; color: var(--heuristic-sabotage); letter-spacing: 3px; font-weight: 800; }
        ...
        .cell.heuristic-active { background: var(--heuristic-sabotage); box-shadow: 0 0 15px var(--heuristic-sabotage); animation: heuristic-wave 1s infinite; }
        .cell.mutation-active { background: var(--mutation-key); box-shadow: 0 0 15px var(--mutation-key); }
        .cell.loading { background: var(--warning); animation: pulse 1s infinite; }

        @keyframes heuristic-wave { 0% { opacity: 1; } 50% { opacity: 0.5; border-radius: 50%; } 100% { opacity: 1; } }
        ...
        .viz-view {
            ...
            grid-template-rows: repeat(5, 70px) 1fr;
            gap: 2px;
        }
        ...
        .log-entry.mutation { border-left-color: var(--mutation-key); color: var(--mutation-key); }
        .log-entry.heuristic { border-left-color: var(--heuristic-sabotage); color: var(--heuristic-sabotage); font-weight: bold; }
        ...
        .btn.active-op-mutation { border-color: var(--mutation-key); background: #831843; color: white; }
        .btn.active-op-heuristic { border-color: var(--heuristic-sabotage); background: #3730a3; color: white; }
...
<body>
    ...
    <header class="nefer-header">
        <div>
            <h1>NEFER-OS // HEURISTIC_PATTERN_SABOTAGE</h1>
            <div style="font-size: 0.6rem; color: var(--text-dim);">41. HÜCRE: SEZGİSEL DESEN SABOTAJI VE DAVRANIŞSAL MASKELEME</div>
        </div>
        <div class="meta-info">
    ...
    <div class="console-container">
        <div class="main-display">
            <div class="viz-view">
                ...
                <div class="viz-box" style="grid-column: span 2;">
                    <div style="position:absolute; top:5px; left:10px; font-size:0.4rem; color:var(--heuristic-sabotage); z-index:5;">HEURISTIC_PATTERN_MASK</div>
                    <canvas id="heuristic-canvas"></canvas>
                    <div id="heuristic-hud" class="hud-text" style="color: var(--heuristic-sabotage);">PATTERN: IDLE | MASK: BENIGN | RISK: LOW</div>
                </div>
        ...
        <aside class="tools-panel">
            <button class="btn" onclick="MutationKeyEngine.toggle()">
                <span>40: CRYPTOGRAPHIC MUTATION KEYING</span>
                <span>DURUM</span>
            </button>
            <button class="btn" id="heuristic-toggle" onclick="HeuristicEngine.toggle()">
                <span>41: HEURISTIC PATTERN SABOTAGE</span>
                <span id="heuristic-status">KAPALI</span>
            </button>
            ...
            <div style="margin-top: auto; font-size: 0.55rem; color: var(--text-dim); border-top: 1px solid var(--border); padding-top: 10px;">
                HÜCRE 41: Şüpheli davranışları meşru kullanıcı işlemleri gibi maskeler.
            </div>
        </aside>
    </div>
    ...
    <footer class="protocol-footer">
        <div class="attribution">
            <p><b>41. HÜCRE: HEURISTIC PATTERN SABOTAGE</b> - Davranışsal Manipülasyon</p>
    ...
    <script type="module">
        const Kernel = {
            ...
            async sequenceBoot() {
                for(let i=1; i<=41; i++) {
                    if([1, 24, 40, 41].includes(i)) continue;
        ...
        const HeuristicEngine = {
            active: false,
            loop: null,
            patterns: ["Auto-save", "Spell-check", "IndexDB Sync", "DOM Render", "Interaction Log"],
            
            toggle() {
                this.active = !this.active;
                const btn = document.getElementById('heuristic-toggle');
                const hud = document.getElementById('heuristic-hud');
                
                if(this.active) {
                    btn.classList.add('active-op-heuristic');
                    document.getElementById('heuristic-status').textContent = "MASKELEME";
                    Kernel.updateCell(41, "heuristic-active");
                    Kernel.log("SEZGİSEL SABOTAJ AKTİF: SALDIRI KALIPLARI MASKELELENİYOR.", "heuristic");
                    this.startViz();
                } else {
                    btn.classList.remove('active-op-heuristic');
                    document.getElementById('heuristic-status').textContent = "KAPALI";
                    Kernel.updateCell(41, "active");
                    cancelAnimationFrame(this.loop);
                    hud.textContent = "PATTERN: IDLE | MASK: BENIGN | RISK: LOW";
                    Kernel.log("Sezgisel sabotaj motoru pasifize edildi.", "sys");
                }
            },
            ...
            startViz() {
                ...
                const draw = () => {
                    ...
                    const pattern = this.patterns[Math.floor(Date.now() / 1000) % this.patterns.length];
                    document.getElementById('heuristic-hud').textContent = `PATTERN: ${pattern} | MASK: ACTIVE | RISK: 0.1%`;

                    // Desen vizüalizasyonu (Meşru trafik görünümlü barlar)
                    for(let i=0; i<10; i++) {
                        const h = 5 + Math.random() * 20;
                        const w = 2 + Math.random() * 15;
                        ctx.fillStyle = "rgba(129, 140, 248, 0.4)";
                        ctx.fillRect(i * 20, c.height - h, w, h);
                    }

                    if(Math.random() > 0.98) {
                        Kernel.log(`Heuristic Sabotaj: Kritik sızıntı "${pattern}" işlemi olarak maskelendi.`, "heuristic");
                        Kernel.log(`Benign No-ops: 500 sahte işlem analiz motoruna beslendi.`, "heuristic");
                    }
        ...
        const MutationKeyEngine = {
            ...
            toggle() {
                this.active = !this.active;
                Kernel.updateCell(40, this.active ? "mutation-active" : "active");
        ...
        const SteganoEngine = {
            ...
            toggle() {
                this.active = !this.active;
                Kernel.updateCell(39, this.active ? "stegano-active" : "active");
                if(this.active) this.start();
            },
            start() {
